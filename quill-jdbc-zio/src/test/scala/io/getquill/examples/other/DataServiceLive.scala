...
/**
 * Not sure why but if you put this into the same class as the caller (e.g.
 * ZioAppExample) then dotty will throw: {{
 * DataService.getPeople.provide(Ctx.dataSourceLayer,
 * (DataServiceLive.apply(_)).toLayer) }}
 */
