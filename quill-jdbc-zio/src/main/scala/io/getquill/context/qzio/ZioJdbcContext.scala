...
abstract class ZioJdbcContext[Dialect <: SqlIdiom, Naming <: NamingStrategy] extends ZioContext[Dialect, Naming]
  ...
  // override def executeAction(sql: String, prepare: Prepare = identityPrepare)(info: ExecutionInfo, dc: Runner): QIO[Long] =
  ...
  // Probe not supported in protoquill yet
  ...
  // This is for TranslateContext which is not supported in ProtoQuill yet
