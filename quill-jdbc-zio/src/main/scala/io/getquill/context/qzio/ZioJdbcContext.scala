 ...
 * import ZioJdbc._ val zioDs = Quill.DataSource.fromPrefix("testPostgresDB")
...
abstract class ZioJdbcContext[+Dialect <: SqlIdiom, +Naming <: NamingStrategy]
  ...
  ): ZIO[DataSource, SQLException, List[T]] = InternalApi.runQuery(quoted, wrap)
  ...
  inline def run[I, A <: Action[I] & QAC[I, Nothing]](
    ...
    rowsPerBatch: Int
  ): ZIO[DataSource, SQLException, List[Long]] = InternalApi.runBatchAction(quoted, rowsPerBatch)
  @targetName("runBatchActionDefault")
  inline def run[I, A <: Action[I] & QAC[I, Nothing]](
    inline quoted: Quoted[BatchAction[A]]
  ): ZIO[DataSource, SQLException, List[Long]] = InternalApi.runBatchAction(quoted, 1)
  @targetName("runBatchActionReturning")
  inline def run[I, T, A <: Action[I] & QAC[I, T]](
    inline quoted: Quoted[BatchAction[A]],
    rowsPerBatch: Int
  ): ZIO[DataSource, SQLException, List[T]] = InternalApi.runBatchActionReturning(quoted, rowsPerBatch)
  @targetName("runBatchActionReturningDefault")
  inline def run[I, T, A <: Action[I] & QAC[I, T]](
    inline quoted: Quoted[BatchAction[A]]
  ): ZIO[DataSource, SQLException, List[T]] = InternalApi.runBatchActionReturning(quoted, 1)

  /**
   * Since we are immediately executing the ZIO that creates this fiber ref
   * whether it is global is not really relevant since it does not really use
   * scope However if it were used for something else it would be scoped to the
   * fiber-ref of the zio-jdbc context's creator i.e. the global scope.
   */
  ...
  override def translateBatchQueryEndpoint(
    groups: List[BatchGroup],
    prettyPrint: Boolean = false
  )(executionInfo: ExecutionInfo, dc: Runner): TranslateResult[List[String]] =
    onConnection(
      connDelegate.translateBatchQueryEndpoint(
