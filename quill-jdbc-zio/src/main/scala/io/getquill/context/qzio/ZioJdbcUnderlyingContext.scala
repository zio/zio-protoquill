...
abstract class ZioJdbcUnderlyingContext[+Dialect <: SqlIdiom, +Naming <: NamingStrategy]
  ...
  inline def run[T](inline quoted: Quoted[Query[T]]): ZIO[Connection, SQLException, List[T]] =
    InternalApi.runQueryDefault(quoted)
  @targetName("runQuery")
  inline def run[T](
    inline quoted: Quoted[Query[T]],
    inline wrap: OuterSelectWrap
  ): ZIO[Connection, SQLException, List[T]] = InternalApi.runQuery(quoted, wrap)
  ...
  inline def run[E](inline quoted: Quoted[Action[E]]): ZIO[Connection, SQLException, Long] =
    InternalApi.runAction(quoted)
  @targetName("runActionReturning")
  inline def run[E, T](inline quoted: Quoted[ActionReturning[E, T]]): ZIO[Connection, SQLException, T] =
    InternalApi.runActionReturning[E, T](quoted)
  @targetName("runActionReturningMany")
  inline def run[E, T](inline quoted: Quoted[ActionReturning[E, List[T]]]): ZIO[Connection, SQLException, List[T]] =
  ...
  inline def run[I, A <: Action[I] & QAC[I, Nothing]](
    ...
    rowsPerBatch: Int
  ): ZIO[Connection, SQLException, List[Long]] = InternalApi.runBatchAction(quoted, rowsPerBatch)
  @targetName("runBatchActionDefault")
  inline def run[I, A <: Action[I] & QAC[I, Nothing]](
    inline quoted: Quoted[BatchAction[A]]
  ): ZIO[Connection, SQLException, List[Long]] = InternalApi.runBatchAction(quoted, 1)
  @targetName("runBatchActionReturning")
  inline def run[I, T, A <: Action[I] & QAC[I, T]](
    inline quoted: Quoted[BatchAction[A]],
    rowsPerBatch: Int
  ): ZIO[Connection, SQLException, List[T]] = InternalApi.runBatchActionReturning(quoted, rowsPerBatch)
  @targetName("runBatchActionReturningDefault")
  inline def run[I, T, A <: Action[I] & QAC[I, T]](
    inline quoted: Quoted[BatchAction[A]]
  ): ZIO[Connection, SQLException, List[T]] = InternalApi.runBatchActionReturning(quoted, 1)

  // Need explicit return-type annotations due to scala/bug#8356. Otherwise macro system will not understand Result[Long]=Task[Long] etc...
  override def executeAction(sql: String, prepare: Prepare = identityPrepare)(
    info: ExecutionInfo,
    dc: Runner
  ): QCIO[Long] =
  ...
  )(info: ExecutionInfo, dc: Runner): QCIO[O] =
  ...
  override def executeActionReturningMany[O](
    ...
    extractor: Extractor[O],
    returningBehavior: ReturnAction
  )(info: ExecutionInfo, dc: Runner): QCIO[List[O]] =
  ...
  override def executeBatchActionReturning[T](groups: List[BatchGroupReturning], extractor: Extractor[T])(
    info: ExecutionInfo,
    dc: Runner
  ): QCIO[List[T]] =
  ...
  override def prepareBatchAction(
    groups: List[BatchGroup]
  )(info: ExecutionInfo, dc: Runner): QCIO[List[PreparedStatement]] =
    super.prepareBatchAction(groups)(info, dc)
  override def translateQueryEndpoint[T](
    statement: String,
    prepare: Prepare = identityPrepare,
    extractor: Extractor[T] = identityExtractor,
    prettyPrint: Boolean = false
  )(info: ExecutionInfo, dc: Runner): QCIO[String] =
    super.translateQueryEndpoint(statement, prepare, extractor, prettyPrint)(info, dc)
  override def translateBatchQueryEndpoint(groups: List[BatchGroup], prettyPrint: Boolean = false)(
    info: ExecutionInfo,
    dc: Runner
  ): QCIO[List[String]] =
  ...
  protected def withConnection[T](f: Connection => Result[T]): Result[T] = throw new IllegalArgumentException(
    "Not Used"
  )
  ...
  /**
   * Note that for ZIO 2.0 since the env is covariant, R can be a subtype of
   * connection because if there are other with-clauses they can be generalized
   * to Something <: Connection. E.g. `Connection with OtherStuff` generalizes
   * to `Something <: Connection`.
   */
        ...
        blocking(
          withoutAutoCommit(
            f.onExit {
              ...
              case Failure(cause) =>
                ZIO
                  .succeed(env.get[Connection].rollback())
                  .foldCauseZIO(
                    ...
                    rollbackFailCause =>
                      ZIO.failCause(cause.flatMap(Cause.die(_, StackTrace.none)) ++ rollbackFailCause),
  ...
  def streamQuery[T](
    ...
    val outStream: ZStream[Connection, Throwable, T] =
      scopedEnv.flatMap { case (conn, ps, rs) =>
