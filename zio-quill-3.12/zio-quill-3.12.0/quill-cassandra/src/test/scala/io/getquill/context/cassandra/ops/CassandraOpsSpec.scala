...
class CassandraOpsSpec extends Spec {
  ...
  "update" - {
    "options" - {
      "timestamp" in {
        inline def q = quote {
          query[TestEntity].usingTimestamp(99).update(lift(TestEntity("s", 1, 2L, None, true))) //hello
        }
        mirrorContext.run(q).string mustEqual
      ...
      "ttl" in {
        inline def q = quote {
          query[TestEntity].usingTtl(1).update(lift(TestEntity("s", 1, 2L, None, true)))
        }
        mirrorContext.run(q).string mustEqual
      ...
      "both" in {
        inline def q = quote {
          query[TestEntity].using(1, 2).update(lift(TestEntity("s", 1, 2L, None, true)))
        }
        mirrorContext.run(q).string mustEqual
