...
class FlicerCombinationSpec extends Spec with Inside {
  ...
  override def beforeAll(): Unit = {
    ...
    ctx.run(
      sql"insert into PersonT (first, last, age) select i, i, i from generate_series(1, 100000) as t(i);"
        .as[Insert[PersonT]]
    )
    ctx.run(
      sql"insert into AddressT (ownerId, street) select i, i from generate_series(1, 100000) as t(i);"
        .as[Insert[PersonT]]
    )
  }
  ...
  "Selection should be correct when" - {
    ...
    "Keys used for filteration are included in the selection output - and they are out of order" in {
      ctx.run(q(List("last", "first", "street"), Map("first" -> "1"))) mustEqual List(
        PersonAddress("1", "1", Some("1"))
      )
    }
