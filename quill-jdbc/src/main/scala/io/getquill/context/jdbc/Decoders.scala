...
trait BasicTimeDecoders extends Decoders {
  ...
  implicit val localDateDecoder: Decoder[LocalDate] =
    decoder((index, row, session) => row.getDate(index).toLocalDate)
  implicit val localTimeDecoder: Decoder[LocalTime] =
    decoder((index, row, session) => row.getTime(index).toLocalTime)
  implicit val localDateTimeDecoder: Decoder[LocalDateTime] =
    decoder((index, row, session) => row.getTimestamp(index).toLocalDateTime)

  implicit val zonedDateTimeDecoder: Decoder[ZonedDateTime] =
    decoder((index, row, session) => ZonedDateTime.ofInstant(row.getTimestamp(index).toInstant, dateTimeZone.toZoneId))
  implicit val instantDecoder: Decoder[Instant] =
    decoder((index, row, session) => row.getTimestamp(index).toInstant)
  ...
  implicit val offsetDateTimeDecoder: Decoder[OffsetDateTime] =
    decoder((index, row, session) => OffsetDateTime.ofInstant(row.getTimestamp(index).toInstant, dateTimeZone.toZoneId))
}
...
trait ObjectGenericTimeDecoders extends Decoders {
  ...
  implicit val localDateDecoder: Decoder[LocalDate] =
    decoder((index, row, session) => row.getObject(index, classOf[LocalDate]))
  implicit val localTimeDecoder: Decoder[LocalTime] =
    decoder((index, row, session) => row.getObject(index, classOf[LocalTime]))
  implicit val localDateTimeDecoder: Decoder[LocalDateTime] =
    decoder((index, row, session) => row.getObject(index, classOf[LocalDateTime]))

  implicit val zonedDateTimeDecoder: Decoder[ZonedDateTime] =
    decoder((index, row, session) => row.getObject(index, classOf[OffsetDateTime]).toZonedDateTime)
  implicit val instantDecoder: Decoder[Instant] =
    decoder((index, row, session) => row.getObject(index, classOf[OffsetDateTime]).toInstant)

  implicit val offsetTimeDecoder: Decoder[OffsetTime] =
    decoder((index, row, session) => row.getObject(index, classOf[OffsetTime]))
  implicit val offsetDateTimeDecoder: Decoder[OffsetDateTime] =
    decoder((index, row, session) => row.getObject(index, classOf[OffsetDateTime]))
}
