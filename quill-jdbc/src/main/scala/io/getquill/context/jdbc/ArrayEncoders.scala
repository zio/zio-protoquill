...
trait ArrayEncoders extends ArrayEncoding {
  ...
  implicit def arrayEncoder[T](implicit core: ArrayCoreEncoder[T, PreparedStatement], ct: ClassTag[T]): Encoder[Array[T]] =
    encoder(ARRAY, (idx: Int, seq: Array[T], row: PreparedStatement) => core.encode(seq, idx, row))

  implicit def seqEncoder[T](implicit core: ArrayCoreEncoder[T, PreparedStatement], ct: ClassTag[T]): Encoder[Seq[T]] =
    encoder(ARRAY, (idx: Int, seq: Array[T], row: PreparedStatement) => core.encode(seq, idx, row)).contramap { (seq: Seq[T]) => seq.toArray }

  implicit def listEncoder[T](implicit core: ArrayCoreEncoder[T, PreparedStatement], ct: ClassTag[T]): Encoder[List[T]] =
    encoder(ARRAY, (idx: Int, seq: List[T], row: PreparedStatement) => core.encode(seq.toArray, idx, row)).contramap { (seq: List[T]) => seq.toArray }

  implicit def setEncoder[T](implicit core: ArrayCoreEncoder[T, PreparedStatement], ct: ClassTag[T]): Encoder[Set[T]] =
    encoder(ARRAY, (idx: Int, seq: List[T], row: PreparedStatement) => core.encode(seq.toArray, idx, row)).contramap { (seq: Set[T]) => set.toArray }


  implicit def stringArrayCoreEncoder: ArrayCoreEncoder[String, PreparedStatement] = encodeRaw(VARCHAR)
  implicit def bigDecimalArrayCoreEncoder: ArrayCoreEncoder[BigDecimal, PreparedStatement] = encodeMapped(_.bigDecimal)
  ...
  implicit def localDateArrayCoreEncoder: ArrayCoreEncoder[LocalDate, PreparedStatement] = encodeMapped(parseJdbcType(DATE), { (ld: LocalDate) => SqlDate.valueOf(ld) })
  implicit def uuidArrayCoreEncoder: ArrayCoreEncoder[UUID, PreparedStatement] = encodeRaw("uuid")

  def encodeRaw[T](arr: Array[T], row: PreparedStatement, sqlType: String): ArrayCoreEncoder[T, PreparedStatement] =
    new ArrayCoreEncoder {
      def encode(arr: Array[T], idx: Int, row: PrepareRow): Unit =
        row.setArray(idx, row.getConnection.createArrayOf(sqlType, arr.asInstanceOf[Array[AnyRef]]))
    }

  def encodeRaw[T](arr: Array[T], row: PreparedStatement, sqlType: Int): ArrayCoreEncoder[T, PreparedStatement] =
    encodeRaw(arr, row, parseJdbcType(sqlType))
