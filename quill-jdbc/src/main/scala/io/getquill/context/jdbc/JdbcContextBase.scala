...
trait JdbcRunContext[Dialect <: SqlIdiom, Naming <: NamingStrategy]
  ...
  // Not overridden in JdbcRunContext in Scala2-Quill because this method is not defined in the context
  override def executeQuery[T](sql: String, prepare: Prepare = identityPrepare, extractor: Extractor[T] = identityExtractor)(
  ...
  override def executeQuerySingle[T](sql: String, prepare: Prepare = identityPrepare, extractor: Extractor[T] = identityExtractor)(
      executionInfo: ExecutionInfo,
      dc: DatasourceContext
  ): Result[T] =
    handleSingleWrappedResult(executeQuery(sql, prepare, extractor)(executionInfo, dc))

  override def executeActionReturning[O](sql: String, prepare: Prepare = identityPrepare, extractor: Extractor[O], returningBehavior: ReturnAction)(
      executionInfo: ExecutionInfo,
      dc: DatasourceContext
  ): Result[O] =
    withConnectionWrapped { conn =>
  ...
  /** Parses instances of java.sql.Types to string form so it can be used in creation of sql arrays. Some databases does not support each of generic types, hence it's welcome to
    * override this method and provide alternatives to non-existent types.
    *
    * @param intType
