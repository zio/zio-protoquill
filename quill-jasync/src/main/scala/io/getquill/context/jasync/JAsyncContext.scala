...
abstract class JAsyncContext[D <: SqlIdiom, +N <: NamingStrategy, C <: ConcreteConnection](
  ...
  override def context: Runner = throw new IllegalStateException(
    "Runner (ExecutionContext) of JAsyncContext is summoned implicitly, the member is unused."
  )
  override def translateContext: TranslateRunner = throw new IllegalStateException(
    "Runner (ExecutionContext) of JAsyncContext is summoned implicitly, the member is unused."
  )
  ...
  )(executionInfo: ExecutionInfo, dc: ExecutionContext): Future[T] =
    implicit val ec = dc
  ...
  def executeAction(
    sql: String,
    prepare: Prepare = identityPrepare
  )(executionInfo: ExecutionInfo, dc: ExecutionContext): Future[Long] = {
    implicit val ec      = dc // implicitly define the execution context that will be passed in
    val (params, values) = prepare(Nil, ())
    logger.logQuery(sql, params)
  ...
  )(executionInfo: ExecutionInfo, dc: ExecutionContext): Future[List[Long]] =
    implicit val ec = dc // implicitly define the execution context that will be passed in
    Future.sequence {
