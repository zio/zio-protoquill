...
class ElaborateProductValueStructureSpec extends Spec {
  ...
  "lambda elaborate construction should work for" - {
    "simple class" in {
      ...
      val lambdas = ElaborateStructureExt.entityValuesLambda[Person]
    ...
    "Nested with optional fields (None)" in {
      ...
      case class Entity(a: String, b: Option[Nested])
      inline def lambdas = ElaborateStructureExt.entityValuesLambda[Entity]
      val values = lambdas.map(l => l(Entity("foo", None)))
      values mustEqual List("foo", None, None)
    }
    ...
    "Quadruple-nested with optional fields" in {
      ...
      case class Entity(a: String, b: Option[Nested])
      inline def lambdas = ElaborateStructureExt.entityValuesLambda[Entity]
      val values = lambdas.map(l => l(Entity("foo", Some(Nested(1, Some(ReallyNested(2, Some(ReallyReallyNested(3, 4)))))))))
      values mustEqual List("foo", Some(1), Some(2), Some(3), Some(4))
    }
  }
