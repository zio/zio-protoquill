...
trait PeopleAggregationSpec extends Spec with SpecEncoders {
  ...
  object `Ex 5 map.groupByMap(col)(col,agg(c)).filter(agg)` {
    /*
    When name is put in here, strange pattern that cannot be parsed:
    |s"==== Tree cannot be parsed to 'Ast' ====
    |  PeopleJdbcAggregationSpec.this
    |==== Extractors ===
    |  This(Some("PeopleJdbcAggregationSpec"))
