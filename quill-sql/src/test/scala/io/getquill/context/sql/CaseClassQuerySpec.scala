...
trait CaseClassQuerySpec extends Spec { self =>
  ...
  inline def `Ex 1A CaseClass Record Output` = quote {
    query[Contact].map(p => ContactSimplified(p.firstName, p.lastName, p.age))
  }

  inline def `Ex 1B CaseClass Record Output` = quote {
  ...
  inline def `Ex 2 Single-Record Join` =
    quote {
      for {
        p <- query[Contact]
