...
trait DistinctSpec extends Spec { self =>
  ...
  given twoFieldDecoder: GenericDecoder[ResultRow, Session, TwoField, DecodingType.Composite] = deriveComposite
  ...
  inline def `Ex 11 DistinctOn With Monadic Join` = quote {
    (for {
      p <- query[Person]
