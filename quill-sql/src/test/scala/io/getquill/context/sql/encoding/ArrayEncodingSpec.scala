...
class ArrayEncodingSpec extends Spec {
  ...
  object impl {
    implicit def encodeRaw[Col <: Iterable[Raw]]: Encoder[Col] = encoder[Col]
    implicit def decodeRaw[Col <: Iterable[Raw]]: Decoder[Col] = decoderUnsafe[Col]
  ...
  "Do not compile in case of upper bound types to Seq or not applicable types" - {
    ...
    "decoders" in {
      import impl.decodeDecor
      implicit val d: MirrorDecoder[Set[Raw]] = decoder[Set[Raw]]
      "implicitly[Decoder[Vector[Decor]]]" mustNot compile
    }
  }
}
