...
class BatchActionTest extends Spec with Inside with SuperContext[MirrorSqlDialectWithReturnClause, Literal] {
  ...
  val ctx: MirrorContext[MirrorSqlDialectWithReturnClause, Literal] =
    new MirrorContext[MirrorSqlDialectWithReturnClause, Literal](MirrorSqlDialectWithReturnClause, Literal)
  import ctx._
  ...
  "batch action with returning should work with" - {
    "insert - returning" in {
      ...
      mirror.triple mustEqual ("INSERT INTO Person (id,name,age) VALUES (?, ?, ?) RETURNING id", List(
        List(1, "Joe", 123),
        List(2, "Jill", 456)
      ), Static)
    }
    ...
    "update - returning" in {
      ...
      mirror.triple mustEqual ("UPDATE Person AS pf SET id = ?, name = ?, age = ? WHERE pf.id = ? RETURNING id", List(
        List(1, "Joe", 123, 1),
        List(2, "Jill", 456, 2)
      ), Static)
    }
  ...
  "batch action should work with" - {
    "dynamic splice" in {
      ...
      mirror.triple mustEqual ("INSERT INTO Person (id,name,age) VALUES (?, ?, ?)", List(
        List(1, "Joe", 123),
        List(2, "Jill", 456)
      ), Dynamic)
    }
    ...
    "insert - different-objects" in {
      ...
      val mirror = ctx.run {
        liftQuery(vips).foreach(v => query[Person].insertValue(Person(v.vipId, v.vipName, v.vipAge)))
      }
      mirror.triple mustEqual ("INSERT INTO Person (id,name,age) VALUES (?, ?, ?)", List(
        List(1, "Joe", 123),
        List(2, "Jill", 456)
    ...
    "update - liftQuery scalars" in {
      ...
      mirror.triple mustEqual ("UPDATE Person AS p SET age = 111 WHERE p.id = ?", List(
        List(1),
        List(2),
        List(3)
      ), Static)
    ...
    "update - liftQuery scalars - dynamic" in {
      val updateDynamic = quote { (i: Int) =>
        query[Person].filter(p => p.id == i).update(_.age -> 111)
      }
    ...
    "update - extra lift" in {
      ...
      mirror.triple mustEqual ("UPDATE Person AS p SET id = ?, name = ?, age = ? WHERE p.id = ?", List(
        List(1, "Joe", 123, 36),
        List(2, "Jill", 456, 36)
      ), Static)
    }
    ...
    "update - extra lift + scalars" in {
      ...
      mirror.triple mustEqual ("UPDATE Person AS p SET age = ? WHERE p.id = ?", List(
        List(1, 36),
        List(2, 36),
        List(3, 36)
      ), Static)
    ...
    "update - extra lift + scalars + multi-use" in {
      val mirror = ctx.run {
        liftQuery(List(1, 2, 3)).foreach(i =>
          query[Person].filter(p => p.id == i && p.age == lift(123)).update(_.age -> i)
        )
      }
      mirror.triple mustEqual ("UPDATE Person AS p SET age = ? WHERE p.id = ? AND p.age = ?", List(
        List(1, 1, 123),
        List(2, 2, 123),
        List(3, 3, 123)
      ), Static)
    ...
    "update - extra lift + scalars + liftQuery/setContains" in {
      val mirror = ctx.run {
        liftQuery(List(1, 2, 3)).foreach(i =>
          query[Person].filter(p => liftQuery(List(36, 49)).contains(p.id)).update(_.age -> i)
        )
      }
      mirror.triple mustEqual ("UPDATE Person AS p SET age = ? WHERE p.id IN (?, ?)", List(
        List(1, 36, 49),
        List(2, 36, 49),
        List(3, 36, 49)
      ), Static)
    ...
    "update - extra lift + scalars + liftQuery/setContains + others" in {
      val mirror = ctx.run {
        liftQuery(List(1, 2, 3)).foreach(i =>
          query[Person].filter(p => liftQuery(List(36, 49)).contains(p.id) && p.id == lift(789)).update(_.age -> i)
        )
      }
      mirror.triple mustEqual ("UPDATE Person AS p SET age = ? WHERE p.id IN (?, ?) AND p.id = ?", List(
        List(1, 36, 49, 789),
        List(2, 36, 49, 789),
        List(3, 36, 49, 789)
      ), Static)
    ...
    "update - extra lift - dynamic" in {
      val updateDynamic = quote { (p: Person) =>
        query[Person].filter(p => p.id == lift(36)).updateValue(p)
      }
    ...
    "update - extra lift - dynamic + scalars" in {
      val updateDynamic = quote { (i: Int) =>
        query[Person].filter(p => p.id == lift(36)).update(_.age -> i)
      }
      val mirror = ctx.run(liftQuery(List(1, 2, 3)).foreach(i => updateDynamic(i)))
      mirror.triple mustEqual ("UPDATE Person AS p SET age = ? WHERE p.id = ?", List(
        List(1, 36),
    ...
    "update via tuple" in {
      ...
      val a = ctx.run {
        liftQuery(birthYearUpdates).foreach { case (id, year) =>
          query[MyPerson].filter(p => p.id == id).update(p => p.birthYear -> year)
        }
      }
      a.triple mustEqual ("UPDATE MyPerson AS p SET birthYear = ? WHERE p.id = ?", List(
        List(1983, 3431),
        List(1972, 2976),
        List(1991, 1511)
      ), Static)
      ...
      b.triple mustEqual ("UPDATE MyPerson AS p SET birthYear = ? WHERE p.id = ?", List(
        List(1983, 3431),
        List(1972, 2976),
        List(1991, 1511)
      ), Static)
    ...
    "update via tuple - dynamic" in {
      val updateDynamic = quote { (id: Int, year: Int) =>
        query[MyPerson].filter(p => p.id == id).update(p => p.birthYear -> year)
      }
    ...
    "update" in {
      val mirror = ctx.run {
        liftQuery(people).foreach(p =>
          query[Person].filter(pf => pf.id == p.id).update(_.name -> p.name, _.age -> p.age)
        )
      }
      mirror.triple mustEqual ("UPDATE Person AS pf SET name = ?, age = ? WHERE pf.id = ?", List(
        List("Joe", 123, 1),
        List("Jill", 456, 2)
      ), Static)
    }
