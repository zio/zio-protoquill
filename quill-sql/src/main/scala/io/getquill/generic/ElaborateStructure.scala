...
/** Elaboration can be different whether we are encoding or decoding because we could have decoders for certain things that we don't have encoders for and vice versa. That means
  * that the potentially something encoded as a value would be decoded as a case-class or vice versa. Therefore, we need to differentiate whether elaboration is used on the
...
/** Based on valueComputation and materializeQueryMeta from the old Quill This was around to flesh-out details of the outermost AST of a query based on the fields of the object T
  * in Query[T] that the AST represents. For an example say we have something like this:
  * {{{
  ...
  * }}}
  * This query needs to be turned into `SELECT p.name, p.age from Person p`, the problem is, before Quats, Quill did not actually know how to expand `Ident("p")` into
  * `SelectValue(p.name), SelectValue(p.age)` (see SqlQuery.scala) since there was no type information. Therefore...
  * {{{
  ...
  * }}}
  *
  * Now, since Quats were introduced into Quill since 3.6.0 (technically since 3.5.3), this step is not necessarily needed for query expansion since `Ident("p")` is now understood
  * to expand into it's corresponding SelectValue fields so for queries, this stage could technically be elimiated. However, this logic is also useful for ActionMeta where we have
  * something like this:
  * {{{
  * case class Person(name: String, age: Int)
  * // This:
...
object ElaborateStructure {
  ...
  object Term {
    ...
    /** Top-Level expansion of a Term is slighly different the later levels. A the top it's always ident.map(id => ...) if Ident is an option as opposed to OptionMap which it would
      * be, in lower layers.
      *
      * Legend: T(x, [y,z]) := Term(x=name, children=List(y,z)), T-Opt=OptionalTerm I.e. term where optional=true P(a, b) := Property(a, b) i.e. a.b M(a, v, P(v, b)) := Map(a, v,
      * P(v, b)) or m.map(v => P(v, b))
      */
  ...
  /** Expand the structure of base term into series of terms for a given type e.g. for Term(x) elaborate Person (case class Person(name: String, age: Int)) will be Term(name,
    * Term(x, Branch), Leaf), Term(age, Term(x, Branch), Leaf) Note that this could potentially be different if we are on the encoding or the decoding side. For example, someone
    * could create something like: {{ case class VerifiedName(val name: String) { ... } val decoding = MappedDecoding ... }} Since we only have decoders, we need to know that
    * VerifiedName is an actual value type as opposed to an embedded case class (since ProtoQuill does not require presence of the 'Embedded' type). So we need to know that
    * VerifiedName is going to be: {{ Term(x, Leaf)) }} as opposed what we would generically have thought: {{ Term(name, Term(x, Branch), Leaf)) }}. That means that we need to know
    * whether to look for an encoder as opposed to a decoder when trying to elaborate this type.
    */
  def base[T: Type](term: Term, side: ElaborationSide)(using Quotes): Term = {
    ...
    // Otherwise, summon the mirror and elaborate the value
      ...
      Expr.summon[Mirror.Of[T]] match
        ...
        case None =>
          report.throwError(
            s"A mirror of the type ${Format.TypeOf[T]} cannot be summoned. It is not a sum-type, a product-type, or a ${encDecText} entity so it's fields cannot be understood in the structure-elaborator."
          )
  }
  ...
  /** Example: case class Person(name: String, age: Option[Int]) val p = Person("Joe") lift(p) Output: Quote(ast: CaseClass("name" -> ScalarTag(name), lifts: EagerLift(p.name,
    * name))
    *
    * Example: case class Name(first: String, last: String) case class Person(name: Name) val p = Person(Name("Joe", "Bloggs")) lift(p) Output: Quote(ast: CaseClass("name" ->
    * CaseClass("first" -> ScalarTag(namefirst), "last" -> ScalarTag(last)), lifts: EagerLift(p.name.first, namefirst), EagerLift(p.name.last, namelast))
    *
    * Note for examples below: idA := "namefirst" idB := "namelast"
    *
    * Example: case class Name(first: String, last: String) case class Person(name: Option[Name]) val p = Person(Some(Name("Joe", "Bloggs"))) lift(p) Output: Quote(ast:
    * CaseClass("name" -> OptionSome(CaseClass("first" -> ScalarTag(idA), "last" -> ScalarTag(idB))), lifts: EagerLift(p.name.map(_.first), namefirst),
    * EagerLift(p.name.map(_.last), namelast))
    *
    * Alternatively, the case where it is: val p = Person(None) the AST and lifts remain the same, only they effectively become None for every value: lifts: EagerLift(None , idA),
    * EagerLift(None , idB))
    *
      ...
      exprType match
        case '[t] =>
          if (TypeRepr.of[t] =:= TypeRepr.of[Any])
            report.warning(
              s"The following the expression was typed `Any`: ${Format.Expr(exprPath)}. Will likely not be able to summon an encoder for this (the actual type was: ${Format.TypeOf[
                T
              ]} in ${Format.TypeRepr(exprPath.asTerm.tpe)})  (the other param was ${Format.TypeOf[T]}."
            )
  ...
  /** Flatten the elaboration from 'node' into a completely flat product type Technicallly don't need Type T but it's very useful to know for errors and it's an internal API so
  ...
  extension [T](opt: Option[T]) def getOrThrow(msg: String) = opt.getOrElse { throw new IllegalArgumentException(msg) }
