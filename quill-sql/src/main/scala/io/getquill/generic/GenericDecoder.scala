...
object Summon {
  def nullChecker[ResultRow: Type, Session: Type](index: Expr[Int], resultRow: Expr[ResultRow])(using
    Quotes
  ): Expr[Boolean] = {
    ...
    Expr.summon[GenericNullChecker[ResultRow, Session]] match {
      ...
      case None              =>
        ...
        report.throwError(s"Cannot find a null-checker for the session type ${Format
            .TypeOf[Session]} (whose result-row type is: ${Format.TypeOf[ResultRow]})")
    }
  }

  def decoder[ResultRow: Type, Session: Type, T: Type](
    index: Expr[Int],
    resultRow: Expr[ResultRow],
    session: Expr[Session]
  )(using Quotes): Option[Expr[T]] = {
  ...
  def decoderOrFail[ResultRow: Type, Session: Type, T: Type](
    index: Expr[Int],
    resultRow: Expr[ResultRow],
    session: Expr[Session]
  )(using Quotes): Expr[T] = {
...
object GenericDecoder {
  ...
  def tryResolveIndex[ResultRow: Type](originalIndex: Expr[Int], resultRow: Expr[ResultRow], fieldName: String)(using
    Quotes
  ) = {
    import quotes.reflect._
  ...
  def flatten[ResultRow: Type, Session: Type, Fields: Type, Types: Type](
    ...
    resultRow: Expr[ResultRow],
    session: Expr[Session]
  )(fieldsTup: Type[Fields], typesTup: Type[Types], accum: List[FlattenData] = List())(using
    Quotes
  ): List[FlattenData] = {
    import quotes.reflect.{Term => QTerm, _}
    ...
    (fieldsTup, typesTup) match {
      ...
      case ('[field *: fields], '[tpe *: types])
          if Expr.summon[GenericDecoder[ResultRow, Session, tpe, DecodingType.Specific]].isEmpty =>
        ...
        flatten[ResultRow, Session, fields, types](nextIndex, baseIndex, resultRow, session)(
          Type.of[fields],
          Type.of[types],
          result +: accum
        )
  ...
  def decodeOptional[T: Type, ResultRow: Type, Session: Type](
    index: Int,
    baseIndex: Expr[Int],
    resultRow: Expr[ResultRow],
    session: Expr[Session]
  )(using Quotes): FlattenData = {
    import quotes.reflect._
    ...
    Expr.summon[GenericDecoder[ResultRow, Session, T, DecodingType.Specific]] match {
      ...
      case Some(_) =>
        val decoder =
          Summon.decoderOrFail[ResultRow, Session, Option[T]]('{ $baseIndex + ${ Expr(index) } }, resultRow, session)
      ...
      case None =>
        val FlattenData(_, construct, nullCheck, lastIndex) =
          decode[T, ResultRow, Session](index, baseIndex, resultRow, session)
  ...
  def decode[T: Type, ResultRow: Type, Session: Type](
    index: Int,
    baseIndex: Expr[Int],
    resultRow: Expr[ResultRow],
    session: Expr[Session],
    overriddenIndex: Option[Expr[Int]] = None
  )(using Quotes): FlattenData = {
    import quotes.reflect._
            ...
            case Some(ev) =>
              ...
              ev match {
                ...
                case '{
                  ...
                  val children =
                    flatten(index, baseIndex, resultRow, session)(Type.of[elementLabels], Type.of[elementTypes]).reverse
                  decodeProduct[T](children, m)

                case _ =>
                  report.throwError(
                    s"Decoder for ${Format.TypeOf[T]} could not be summoned. It has no decoder and is not a recognized Product or Sum type."
                  )
              } // end match
            case _ =>
              report.throwError(
  ...
  def summon[T: Type, ResultRow: Type, Session: Type](using
    quotes: Quotes
  ): Expr[GenericDecoder[ResultRow, Session, T, DecodingType.Generic]] = {
    import quotes.reflect._
    '{
      new GenericDecoder[ResultRow, Session, T, DecodingType.Generic] {
        def apply(baseIndex: Int, resultRow: ResultRow, session: Session) = ${
          GenericDecoder.decode[T, ResultRow, Session](0, 'baseIndex, 'resultRow, 'session).decodedExpr
        }.asInstanceOf[T]
      }
    }
  }
...
object DecodeSum {
  def apply[T: Type, ResultRow: Type, Session: Type, ElementTypes: Type](
    index: Int,
    baseIndex: Expr[Int],
    resultRow: Expr[ResultRow],
    session: Expr[Session]
    ...
    Expr.summon[GenericColumnResolver[ResultRow]] match {
      case None =>
        report.warning(
          s"Need column resolver for in order to be able to decode a coproduct but none exists for ${Format
              .TypeOf[T]} (row type: ${Format.TypeOf[ResultRow]}). " +
      ...
      case _ =>
        ...
        Expr.summon[GenericRowTyper[ResultRow, T]] match {
          case Some(rowTyper) =>
            ...
            selectMatchingElementAndDecode[ElementTypes, ResultRow, Session, T](
              index,
              baseIndex,
              resultRow,
              session,
          ...
          case None =>
            ...
            // the user actually has actionable information on how to resolve the problem.
            report.warning(
              s"Need a RowTyper for ${Format.TypeOf[T]}. Have you implemented a RowTyper for it? Otherwise the decoder will fail at runtime if this type is encountered"
            )
  ...
  /**
   * Find a type from a coproduct type that matches a given ClassTag, if it
   * matches, summon a decoder for it and decode it
   */
  def selectMatchingElementAndDecode[Types: Type, ResultRow: Type, Session: Type, T: Type](
    index: Int,
    rawIndex: Expr[Int],
    resultRow: Expr[ResultRow],
    session: Expr[Session],
    rowTypeClassTag: Expr[ClassTag[_]]
  )(typesTup: Type[Types])(using
    Quotes
  ): FlattenData = {
    import quotes.reflect._
    typesTup match {
      case ('[tpe *: types]) =>
        println(
          s"(Co-Product) Checking if ${Format.TypeOf[tpe]} == ${Format.Expr(rowTypeClassTag)} and should be spliced into index: ${index}"
        )
        ...
        // case class Circle(radius: Int) extends Shape
        // In that case `tpe` here will be Square/Circle
        val thisElementDecoder =
          GenericDecoder.decode[tpe, ResultRow, Session](index, rawIndex, resultRow, session).decodedExpr

        val thisElementNullChecker = '{ !${ Summon.nullChecker[ResultRow, Session](rawIndex, resultRow) } }
        // make the recursive call
        val nextData = selectMatchingElementAndDecode[types, ResultRow, Session, T](
          index + 1,
          rawIndex,
          resultRow,
          session,
