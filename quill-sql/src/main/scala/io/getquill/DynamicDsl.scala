...
private[getquill] def dyn[T](
  ast: Ast,
  lifts: List[Planter[_, _, _]],
  runtimeQuotes: List[QuotationVase]
): DynamicQuery[T] =
  DynamicQuery[T](Quoted[Query[T]](ast, lifts, runtimeQuotes))

private[getquill] def spliceLift[O](o: O, otherLifts: List[Planter[_, _, _]], runtimeQuotes: List[QuotationVase])(
