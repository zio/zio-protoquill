...
object Extractors {
  ...
  object SelectApplyN {
    ...
    object Term:
      ...
      ): Option[(quotes.reflect.Term, String, List[quotes.reflect.Term])] =
        import quotes.reflect._
        term match
  ...
  object Lambda1 {
    ...
    object Term {
      def unapply(using Quotes)(
        term: quotes.reflect.Term
      ): Option[(String, quotes.reflect.TypeRepr, quotes.reflect.Term)] =
        import quotes.reflect._
        Untype(term) match {
  ...
  object Lambda2 {
    def unapply(using Quotes)(
      expr: Expr[_]
    ): Option[(String, quotes.reflect.TypeRepr, String, quotes.reflect.TypeRepr, quoted.Expr[_])] =
      import quotes.reflect._
  ...
  object RawLambdaN {
    def unapply(using Quotes)(
      term: quotes.reflect.Term
    ): Option[(List[(String, quotes.reflect.TypeRepr)], quotes.reflect.Term)] =
      import quotes.reflect._
      Untype(term) match {
  ...
  def nestInline(using Quotes)(call: Option[quotes.reflect.Tree], defs: List[quotes.reflect.Definition])(
    expr: Expr[_]
  ): Expr[_] =
    import quotes.reflect._
