...
object MapFlicer {
  ...
  private def applyImpl[T: Type, PrepareRow: Type, Session: Type](entity: Expr[T], map: Expr[Map[String, Any]])(using
    Quotes
  ): Expr[Boolean] = {
...
class MapFlicerMacro {
  ...
  private def buildClause[T: Type, PrepareRow: Type, Session: Type](
    core: Expr[T]
  )(map: Expr[Map[String, Any]])(using Quotes): Expr[Boolean] = {
    import quotes.reflect._
          ...
          terms.map { (fieldString, isOptional, getter, tpe) =>
            ...
            def fieldInject[T](field: Expr[T])(using Type[T]) = {
              ...
              '{
                ...
                $field == ${ LiftMacro.valueOrString[T, PrepareRow, Session](mapSplice) } || ${
                  LiftMacro[Boolean, PrepareRow, Session]('{ $mapSplice == null })
                }
              }
            }
            ...
            tpe match {
              case '[inner] =>
                ...
                if (isOptional)
                  '{
                    ${ childTTerm.asExprOf[Option[inner]] }.exists(field => ${ fieldInject[inner]('{ field }) })
                  }
                else
  ...
  def base[T, PrepareRow, Session](using
    Quotes
  )(expr: Expr[T], map: Expr[Map[String, Any]])(using
    tpe: Type[T],
    pr: Type[PrepareRow],
    sess: Type[Session]
  ): Expr[Boolean] = {
