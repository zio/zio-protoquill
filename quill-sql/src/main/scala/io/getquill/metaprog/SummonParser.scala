...
object SummonSerializationBehaviors:
  ...
  /**
   * Summon any serialization behavior defined on the context. If it does not
   * exist return None
   */
        ...
        factoryTypeRepr.asType match
          case '[t] =>
            Load.Module[t] match
              ...
              case Failure(e) =>
                report.throwError(
                  s"Could not summon a parser of type ${Format.TypeOf[t]}. A parser must be a static object created in it's own compilation unit. ${e}"
                )
      case None =>
