...
/**
 * Trait that allows usage of 'static' block. Can declared one of these and use
 * similar to encoders but it needs to be compiled in a previous compilation
 * unit and a global static. TODO More explanation
 */
trait ToSql[T] {
...
object StringCodec {
  object ToSql {
    def summon[T: Type](using Quotes): Either[String, ToSql[T]] = {
      ...
      for {
        summonValue <-
          Expr.summon[io.getquill.ToSql[T]].toEitherOr(s"a ToString[${Format.TypeOf[T]}] cannot be summoned")
