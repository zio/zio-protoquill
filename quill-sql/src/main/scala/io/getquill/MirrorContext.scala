...
class MirrorContext[+Dialect <: Idiom, +Naming <: NamingStrategy](
  val idiom: Dialect,
  val naming: Naming,
...
trait MirrorContextBase[+Dialect <: Idiom, +Naming <: NamingStrategy]
  ...
  inline def run[E, T](inline quoted: Quoted[ActionReturning[E, T]]): ActionReturningMirror[T, T] =
    InternalApi.runActionReturning(quoted).asInstanceOf[ActionReturningMirror[T, T]]
  ...
  inline def run[E, T](inline quoted: Quoted[ActionReturning[E, List[T]]]): ActionReturningMirror[T, List[T]] =
    InternalApi.runActionReturningMany(quoted).asInstanceOf[ActionReturningMirror[T, List[T]]]
  @targetName("runBatchAction")
  inline def run[I, A <: Action[I] & QAC[I, Nothing]](
    inline quoted: Quoted[BatchAction[A]],
    rowsPerBatch: Int
  ): BatchActionMirror = InternalApi.runBatchAction(quoted, rowsPerBatch)
  ...
  inline def run[I, T, A <: Action[I] & QAC[I, T]](
    inline quoted: Quoted[BatchAction[A]],
    rowsPerBatch: Int
  ): BatchActionReturningMirror[T] = InternalApi.runBatchActionReturning(quoted, rowsPerBatch)
  @targetName("runBatchActionReturningDefault")
  inline def run[I, T, A <: Action[I] & QAC[I, T]](
    inline quoted: Quoted[BatchAction[A]]
  ): BatchActionReturningMirror[T] = InternalApi.runBatchActionReturning(quoted, 1)
  ...
  override def executeAction(string: String, prepare: Prepare = identityPrepare)(
    info: ExecutionInfo,
    dc: Runner
  ...
  def executeActionReturning[T](
    ...
    extractor: Extractor[T],
    returningBehavior: ReturnAction
  )(info: ExecutionInfo, dc: Runner): Result[RunActionReturningResult[T]] =
  ...
  def executeActionReturningMany[T](
    ...
    extractor: Extractor[T],
    returningBehavior: ReturnAction
  )(info: ExecutionInfo, dc: Runner): Result[RunActionReturningResult[List[T]]] =
  ...
  )(info: ExecutionInfo, dc: Runner): Result[RunBatchActionResult] =
    BatchActionMirror(
  ...
  override def executeBatchActionReturning[T](
    ...
    extractor: Extractor[T]
  )(info: ExecutionInfo, dc: Runner): Result[RunBatchActionReturningResult[T]] =
