...
object Mac {
  ...
  private def enterImpl(str: Expr[String])(using Quotes): Expr[Unit] = {
  ...
  private def passThroughImpl(str: Expr[String])(using Quotes): Expr[String] = {
    import quotes.reflect._
...
object MatchMac {
    ...
    private def printMacImpl(anyRaw: Expr[Any])(using Quotes): Expr[Unit] = {
      class Operations(implicit val qctx: Quotes) {
