...
object Format {
  ...
  def apply(code: String) = {
    ...
    val formattedCode =
      for {
        cls <- Try { /* println("getting formatter"); */
          Class.forName("org.scalafmt.cli.Scalafmt210")
        }.toOptionMsg
        inst <- Try { /* println("making instance"); */
          cls.newInstance()
        }.toOptionMsg
        formatMethod <- Try { /* println("getting format method"); */
          cls.getMethod("format", classOf[String], classOf[String])
        }.toOptionMsg
        formatted <- Try { /* println("formatting"); */
          formatMethod.invoke(inst, encosedCode, "Main.scala")
        }.toOptionMsg
