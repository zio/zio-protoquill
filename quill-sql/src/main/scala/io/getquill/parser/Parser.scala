...
trait ParserLibrary extends ParserFactory {
  ...
  def quotationParser(using Quotes)          = Series.single(new QuotationParser)
  def queryParser(using Quotes)              = Series.single(new QueryParser)
  def queryScalarsParser(using Quotes)       = Series.single(new QueryScalarsParser)
  def patMatchParser(using Quotes)           = Series.single(new CasePatMatchParser)
  def functionParser(using Quotes)           = Series.single(new FunctionParser)
  def functionApplyParser(using Quotes)      = Series.single(new FunctionApplyParser)
  def valParser(using Quotes)                = Series.single(new ValParser)
  def blockParser(using Quotes)              = Series.single(new BlockParser)
  def operationsParser(using Quotes)         = Series.single(new OperationsParser)
  def orderingParser(using Quotes)           = Series.single(new OrderingParser)
  def genericExpressionsParser(using Quotes) = Series.single(new GenericExpressionsParser)
  def actionParser(using Quotes)             = Series.single(new ActionParser)
  def batchActionParser(using Quotes)        = Series.single(new BatchActionParser)
  def optionParser(using Quotes)             = Series.single(new OptionParser)
  def valueParser(using Quotes)              = Series.single(new ValueParser)
