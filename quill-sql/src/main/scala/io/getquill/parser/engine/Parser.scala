...
object ParserNodeCache {
  val cache: LRUCache[(String, Expr[_]), Option[Ast]] = new LRUCache(ProtoMessages.cacheParserMax + 1) // add one since capacity 0 will crash creation
...
trait Parser(rootParse: Parser | Parser.Nil)(using Quotes) {
  ...
  private[engine] def attemptProper: History ?=> Function[Expr[_], Option[Ast]] = { (expr: Expr[_]) =>
    ...
    if (ProtoMessages.cacheParser && ProtoMessages.cacheParserMax > 0)
