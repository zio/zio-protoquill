...
object ParserHelpers:
  ...
  trait Assignments extends Idents:
    ...
    object AssignmentTerm:
      ...
      object TwoComponents:
        def unapply(expr: Expr[_])(using Quotes) =
          UntypeExpr(expr) match
            case Lambda2(ident1, identTpe1, ident2, identTpe2, ArrowFunction(prop, value)) =>
              Some((ident1, identTpe1, ident2, identTpe2, prop, value))
            case _ => None
        ...
        def checkPropAndValue(parent: Expr[Any], prop: Expr[Any], value: Expr[Any])(using Quotes) =
          ...
          if (isNumericPrimitive(propTpe) && isNumericPrimitive(valueTpe)) {
            if (!(numericPrimitiveFitsInto(propTpe, valueTpe))) {
              report.throwError(
                s"The primitive numeric value ${Format.TypeRepr(valueTpe)} in ${Format
                    .Expr(value)} is to large to fit into the ${Format.TypeRepr(propTpe)} in ${Format.Expr(prop)}.",
                parent
              )
            }
          }
          // Otherwise check if the property is a subtype of the value that is being assigned to it
            ...
            report.throwError(
              s"The ${Format.TypeRepr(valueTpe)} value ${Format.Expr(value)} cannot be assigned to the ${Format
                  .TypeRepr(propTpe)} property ${Format.Expr(prop)} because they are not the same type (or a subtype).",
              parent
            )
          }
  ...
  /**
   * Helpers for different behaviors Quill supports of object equality. This is
   * non-trivial since Quill has to make sense of different equality paradigms
   * across ANSI-SQL and Scala for objects that may be Optional or not. Several
  ...
  trait ComparisonTechniques:
    ...
    /**
     * Taken from the identically named method in Parser.scala in Scala2-Quill.
     * Much of this logic is not macro specific so a good deal of it can be
     * refactored out into the quill-sql-portable module. Do equality checking
     * on the database level with the same truth-table as idiomatic scala
     */
    def equalityWithInnerTypechecksIdiomatic(using
      Quotes,
      History
    )(left: quotes.reflect.Term, right: quotes.reflect.Term)(equalityBehavior: EqualityBehavior) =
      import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
    ...
    /**
     * (not used yet but will be used when support for 'extras' dsl
     * functionality is added) Do equality checking on the database level with
     * the ansi-style truth table (i.e. always false if one side is null)
     */
    def equalityWithInnerTypechecksAnsi(using Quotes, History)(left: quotes.reflect.Term, right: quotes.reflect.Term)(
     ...
     * inside. If they are not compareable, abort the build. Otherwise return
     * type of which side (or both) has the optional. In order to do the actual
     ...
     */
    def checkInnerTypes(using Quotes)(
      ...
      val leftIsOptional =
        isOptionType(leftType) && !(leftType.widen =:= TypeRepr.of[Nothing]) && !(leftType.widen =:= TypeRepr.of[Null])
      val rightIsOptional =
        isOptionType(rightType) && !(rightType.widen =:= TypeRepr.of[Nothing]) && !(rightType.widen =:= TypeRepr
          .of[Null])
      ...
      optionCheckBehavior match
        ...
        case _ =>
          if (leftIsOptional || rightIsOptional)
            report.throwError(
              s"${Format.TypeReprW(leftType)} == ${Format.TypeReprW(rightType)} is not allowed since ${Format
                  .TypeReprW(leftInner)}, ${Format.TypeReprW(rightInner)} are different types.",
              lhs.asExpr
            )
          else
            report.throwError(
              s"${Format.TypeReprW(leftType)} == ${Format.TypeReprW(rightType)} is not allowed since they are different types.",
              lhs.asExpr
            )
    ...
    /**
     * Match types in the most wide way possible. This function is not for
     * generalized type equality since quill does not directly compare anything,
     * rather it just translates things into SQL expressions. This kind of check
     * is used in a general sense when things that it doesn't even make sense to
     * compare are compared e.g. an Person and a String. In this case, we want
     * to provide some kind of compile-time warning that the comparision the
     * user is attempting to do in SQL is non sensical in the first place.
     * Therefore when there is any kind of possibility that the expression makes
     * sense (e.g. by comparing a Dog to a Animal (i.e. class to subclass), by
     * comparing two numeric types of any kind etc... we allow the comparison to
     * happen). For int/long/float/double comparisons don't crash on
     * compile-time typing can re-evaluate this upon user feedback
     */
  ...
  trait PatternMatchingValues extends Parser with QuatMaking:
          ...
          case DefDef(name, paramss, tpe, rhsOpt) if (paramss.length == 0) =>
            ...
            val body =
              rhsOpt match {
                ...
                case None =>
                  report.throwError(
                    s"Cannot parse 'val' clause with no '= rhs' (i.e. equals and right hand side) of ${Printer.TreeStructure
                        .show(tree)}"
                  )
                case Some(rhs) => rhs
              }
      ...
      def unapply(using Quotes, History, TranspileConfig)(root: quotes.reflect.Term): Option[PatMatch] =
        ...
        root match
          ...
          case m @ Match(expr, caseDefs) =>
            ...
            val clauses =
              caseDefs.map { case CaseDef(fields, guard, body) =>
                val bodyAst = betaReduceTupleFields(expr, fields, Some(root))(body)
                val guardAst =
                  guard.map(betaReduceTupleFields(expr, fields)(_)).getOrElse(ast.Constant(true, Quat.BooleanValue))
    ...
    /**
     * Beta-reduces out tuple members that have been pattern matched to their
     * corresponding components For example: given: ptups := people.map(p =>
     * (p.name, p.age)) ptups.map { case (name, age) => fun(name, age) } becomes
     * reduced to: ptups.map { x => fun(x.name, x.age) }
     */
    protected def betaReduceTupleFields(using
      Quotes,
      History,
      TranspileConfig
    )(tupleTree: quotes.reflect.Term, fieldsTree: quotes.reflect.Tree, messageExpr: Option[quotes.reflect.Term] = None)(
      bodyTree: quotes.reflect.Term
    ): Ast = {
      import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
          ...
          case other =>
            val addition =
              messageExpr match
                ...
                case None       => ""
            report.throwError(
              s"Invalid Pattern Matching Term: ${Format.Tree(other)}${addition}.\n" +
      ...
      def propertyAt(path: List[String]) =
        path.foldLeft(tuple) { case (tup, elem) =>
          Property(tup, elem)
        }
  ...
  object ImplicitClassExtensionPattern:
    ...
    def unapply(expr: Expr[_])(using Quotes) =
      ...
      expr match
        ...
        case expr @ Unseal(UntypeApply(cc @ Apply(UntypeApply(ccid), List(constructorArg))))
            if (isImplicitClassMaker(cc) && isImplicitClassMethod(ccid)) =>
