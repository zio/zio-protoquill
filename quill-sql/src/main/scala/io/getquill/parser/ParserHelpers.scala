...
object ParserHelpers {
  ...
  trait Assignments extends Idents {
    ...
    object AssignmentTerm {
      ...
      def OrFail(expr: Expr[_])(using Quotes, History, Lifts) =
        unapply(expr).getOrElse { failParse(expr, classOf[Assignment]) }

      def unapply(expr: Expr[_])(using Quotes, History, Lifts): Option[Assignment] =
        UntypeExpr(expr) match {
      ...
      object Double {
        ...
        def unapply(expr: Expr[_])(using Quotes, History, Lifts): Option[AssignmentDual] =
          UntypeExpr(expr) match {
  ...
  trait PropertyParser(implicit val qctx: Quotes) {
    ...
    object LambdaToProperty {
      object OrFail {
        def apply(expr: Expr[_])(using History, Lifts): Property =
          unapply(expr) match {
      ...
      def unapply(expr: Expr[_])(using History, Lifts): Option[Property] =
        expr match {
  ...
  trait ComparisonTechniques {
     ...
     */
    def equalityWithInnerTypechecksIdiomatic(using Quotes, History, Lifts)(left: quotes.reflect.Term, right: quotes.reflect.Term)(equalityBehavior: EqualityBehavior) = {
      import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
     ...
     */
    def equalityWithInnerTypechecksAnsi(using Quotes, History, Lifts)(left: quotes.reflect.Term, right: quotes.reflect.Term)(equalityBehavior: EqualityBehavior) = {
      import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
      import io.getquill.ast.Implicits._
  ...
  trait PatternMatchingValues extends Parser with QuatMaking {
    ...
    object ValDefTerm {
      def unapply(using Quotes, History, Lifts, TranspileConfig)(tree: quotes.reflect.Tree): Option[Ast] = {
        import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
        tree match {
    ...
    object PatMatchTerm {
      object SimpleClause {
        def unapply(using Quotes, History, Lifts, TranspileConfig)(term: quotes.reflect.Term): Option[Ast] =
      ...
      def unapply(using Quotes, History, Lifts, TranspileConfig)(root: quotes.reflect.Term): Option[PatMatch] = {
        import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
        root match {
     ...
     */
    protected def betaReduceTupleFields(using Quotes, History, Lifts, TranspileConfig)(tupleTree: quotes.reflect.Term, fieldsTree: quotes.reflect.Tree, messageExpr: Option[quotes.reflect.Term] = None)(bodyTree: quotes.reflect.Term): Ast = {
      import quotes.reflect.{Ident => TIdent, ValDef => TValDef, _}
