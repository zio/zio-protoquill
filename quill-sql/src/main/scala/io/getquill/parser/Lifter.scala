 ...
 * back an fourth between inline Quotation blocks during compile-time which
 * should eventually be bassed into a run-call-site where they will be evaluated
 * into SQL.
 *
...
case class Lifter(serializeQuat: SerializeQuat, serializeAst: SerializeAst) extends Lifters.Proxy {
  ...
  trait LiftAstSerialize[T <: Ast: ClassTag]
      extends ToExpr[T]
      with Lifters.WithSerializing.Ast[T]
      with Lifters.Plain.Ast[T] {
  ...
  trait LiftQuatSerialize[T <: Quat: ClassTag]
      extends ToExpr[T]
      with Lifters.WithSerializing.Quat[T]
      with Lifters.Plain.Quat[T] {
    def typeTag: Quotes ?=> TType[T]
    def lift: Quotes ?=> PartialFunction[T, Expr[T]]
  ...
  given liftIdent: LiftAstSerialize[AIdent] with {
    ...
    def lift = { case AIdent.Opinionated(name: String, quat, visibility) =>
      '{ AIdent.Opinionated(${ name.expr }, ${ quat.expr }, ${ visibility.expr }) }
    }
  }

  given liftPropertyAlias: Lifters.Plain[PropertyAlias] with {
    def lift = { case PropertyAlias(a, b) =>
      '{ PropertyAlias(${ a.expr }, ${ b.expr }) }
    }
  }

  given liftAssignment: LiftAstSerialize[Assignment] with {
    def typeTag = TType.of[Assignment]
    def lift = { case Assignment(ident, property, value) =>
      '{ Assignment(${ ident.expr }, ${ property.expr }, ${ value.expr }) }
    }
  }

  given liftAssignmentDual: LiftAstSerialize[AssignmentDual] with {
    def typeTag = TType.of[AssignmentDual]
    def lift = { case AssignmentDual(ident1, ident2, property, value) =>
      '{ AssignmentDual(${ ident1.expr }, ${ ident2.expr }, ${ property.expr }, ${ value.expr }) }
    }
  }
  ...
  given liftQuatProduct: LiftQuatSerialize[Quat.Product] with {
    ...
    def lift = { case Quat.Product.WithRenamesCompact(name, tpe, fields, values, renamesFrom, renamesTo) =>
      '{
        io.getquill.quat.Quat.Product.WithRenamesCompact.apply(${ name.expr }, ${ tpe.expr })(${
          fields.toList.spliceVarargs
        }: _*)(${ values.toList.spliceVarargs }: _*)(${ renamesFrom.toList.spliceVarargs }: _*)(${
  ...
  given liftEntity: LiftAstSerialize[Entity] with {
    ...
    def lift = {
      ...
      case Entity.Opinionated(name: String, list, quat, renameable) =>
        '{ Entity.Opinionated(${ name.expr }, ${ list.expr }, ${ quat.expr }, ${ renameable.expr }) }
    }
  }

  given liftCaseClass: LiftAstSerialize[CaseClass] with {
    def typeTag = TType.of[CaseClass]
    def lift = { case cc @ CaseClass(name, lifts) =>
      '{
        CaseClass(${ name.expr }, ${ lifts.expr })
      } // List lifter and tuple lifter come built in so can just do Expr(lifts) (or lifts.expr for short)
    }
  }

  given liftTuple: LiftAstSerialize[Tuple] with {
    def typeTag = TType.of[Tuple]
    def lift = { case Tuple(values) =>
      '{ Tuple(${ values.expr }) }
    }
  }
  ...
  given liftAction: LiftAstSerialize[Action] with {
    ...
    def lift = {
      ...
      case Returning(action: Ast, alias: AIdent, body: Ast) =>
        '{ Returning(${ action.expr }, ${ alias.expr }, ${ body.expr }) }
      case ReturningGenerated(action: Ast, alias: AIdent, body: Ast) =>
        '{ ReturningGenerated(${ action.expr }, ${ alias.expr }, ${ body.expr }) }
      case Foreach(query: Ast, alias: AIdent, body: Ast) =>
        '{ Foreach(${ query.expr }, ${ alias.expr }, ${ body.expr }) }
  ...
  given liftQuery: LiftAstSerialize[AQuery] with {
    ...
    def lift = {
      ...
      case FlatMap(query: Ast, alias: AIdent, body: Ast) =>
        '{ FlatMap(${ query.expr }, ${ alias.expr }, ${ body.expr }) }
      case ConcatMap(query: Ast, alias: AIdent, body: Ast) =>
        '{ ConcatMap(${ query.expr }, ${ alias.expr }, ${ body.expr }) }
      case SortBy(query: Ast, alias: AIdent, criterias: Ast, ordering: Ast) =>
        '{ SortBy(${ query.expr }, ${ alias.expr }, ${ criterias.expr }, ${ ordering.expr }) }
      case GroupBy(query: Ast, alias: AIdent, body: Ast) =>
        '{ GroupBy(${ query.expr }, ${ alias.expr }, ${ body.expr }) }
      ...
      case Join(typ, a, b, identA, identB, body) =>
        '{ Join(${ typ.expr }, ${ a.expr }, ${ b.expr }, ${ identA.expr }, ${ identB.expr }, ${ body.expr }) }
  ...
  given liftAst: LiftAstSerialize[Ast] with {
    ...
    def lift = {
      ...
      case UnaryOperation(operator: UnaryOperator, a: Ast) =>
        '{ UnaryOperation(${ liftOperator(operator).asInstanceOf[Expr[UnaryOperator]] }, ${ a.expr }) }
      case BinaryOperation(a: Ast, operator: BinaryOperator, b: Ast) =>
        '{ BinaryOperation(${ a.expr }, ${ liftOperator(operator).asInstanceOf[Expr[BinaryOperator]] }, ${ b.expr }) }
      case v: ScalarTag    => liftScalarTag(v)
      case v: QuotationTag => liftQuotationTag(v)
      case Infix(parts, params, pure, transparent, quat) =>
        '{ Infix(${ parts.expr }, ${ params.expr }, ${ pure.expr }, ${ transparent.expr }, ${ quat.expr }) }
  ...
  given liftScalarTag: LiftAstSerialize[ScalarTag] with {
    ...
    def lift = { case ScalarTag(uid: String, source) =>
      '{ ScalarTag(${ uid.expr }, ${ source.expr }) }
    }
  }

  given liftQuotationTag: LiftAstSerialize[QuotationTag] with {
    def typeTag = TType.of[QuotationTag]
    def lift = { case QuotationTag(uid: String) =>
      '{ QuotationTag(${ uid.expr }) }
    }
  }
  ...
  given liftOperator: Lifters.Plain[Operator] with {
    def lift = {
      ...
      case EqualityOperator.`_==` =>
        '{ EqualityOperator.`_==` } // if you don't do it this way, complains about 'stable identifier error'
      case EqualityOperator.`_!=` =>
        '{
          EqualityOperator.`_!=`
        } // (can't use 'ne' here because 'ne' alias is a non-stable identifier? maybe used for something else?)
