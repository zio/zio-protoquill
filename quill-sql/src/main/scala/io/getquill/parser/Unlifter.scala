        ...
        case _ =>
          unlift.lift(expr).getOrElse {
            throw new IllegalArgumentException(
              s"Could not Unlift AST type ${classTag[T].runtimeClass.getSimpleName} from the element ${pprint.apply(quotes.reflect.asTerm(expr))} into the Quill Abstract Syntax Tree"
            )
          }

    /** For things that contain subclasses, don't strictly check the super type and fail the match if the type fails since we can't do that when it could be a sub-type of the type
      * being matched. The only thing we can do in that case is go through the sub-parser and see if anything matches.
      */
