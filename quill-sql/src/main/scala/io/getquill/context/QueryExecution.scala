...
object QueryExecution:
  ...
  trait SummonHelper[ResultRow: Type] {
    ...
    def summonDecoderOrThrow[DecoderT: Type]()(using Quotes): Expr[GenericDecoder[ResultRow, DecoderT]] = {
      Expr.summon[GenericDecoder[ResultRow, DecoderT]] match {
        case Some(decoder) => decoder
        case None =>
          quotes.reflect.report.throwError(s"Decoder could not be summoned during query execution for the type ${io.getquill.util.Format.TypeOf[DecoderT]}")
      }
    }
  }
  ...
  extends SummonHelper[ResultRow]
    ...
    def apply() =
      QAC match
        ...
        case _ =>
          quotes.reflect.report.throwError(s"Could not match type type of the quoted operation: ${io.getquill.util.Format.Type(QAC)}")
    ...
    def resolveLazyLifts(lifts: List[Expr[Planter[?, ?]]])(using Quotes): List[Expr[EagerPlanter[?, ?]]] =
