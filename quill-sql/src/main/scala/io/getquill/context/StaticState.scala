...
case class StaticState(query: Unparticular.Query, rawLifts: List[PlanterExpr[?, ?, ?]], returnAction: Option[ReturnAction], idiom: Idiom)(queryAst: => Ast):
  /** Plant all the lifts and return them. NOTE: If this is used frequently would it be worth caching (i.e. since this object is immutable) and splicing them might be expensive if
