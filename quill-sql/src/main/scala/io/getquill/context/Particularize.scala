...
/** For a query that has a filter(p => liftQuery(List("Joe","Jack")).contains(p.name)) we need to turn the "WHERE p.name in (?)" into WHERE p.name in (?, ?) i.e. to "Particularize"
  * the query to the number of elements in the query lift. In Scala2-Quill we could just access the values of the liftQuery list directly since the lift was an 'Any' value directly
  * in the AST. In Scala 3 however, we need to treat the lifted list as an Expr and create an Expr[String] that represents the Query that is to be during runtime based on the
  * content of the list which has to be manipulated inside of a '{ ... } block.
  */
object Particularize:
  object Static:
    ...
    private[getquill] def raw[PrepareRowTemp, Session](
        statement: Statement,
        lifts: List[Expr[Planter[_, _, _]]],
        runtimeLiftingPlaceholder: Expr[Int => String],
        emptySetContainsToken: Token => Token
      ...
      /** Actual go from a liftQuery(List("Joe", "Jack")) to "?, ?" using the lifting placeholder. Also return how much the index should be incremented
        */
      ...
      /** A Matrowl lit. Matroshka + Bowl is essentially a stack where each frame consists of a list of items. You can add to the list on the top of the stack, pop the current
        * Matrowl, or stack another one on top of it. This datastructure became necessary in the token2Expr function when I realized in that in the case of:
        * Work.Token(SetContainsToken(a, op, b @ ScalarTagToken(tag))), the list this tag points to can be empty which means that the emptySetContainsToken needs to be expanded
        * instead of the Expr[String] that is returned by the placeholders function. The problem however is that we only know at runtime whether the list is zero or non-zero
        * elements long. This leads us to the requirement to either make token2Expr on tail recursive and introduce a something like this: {{ case Work.Token(SetContainsToken(a,
        * op, b @ ScalarTagToken(tag))) => '{if (list.length != 0) token2Expr(...) else token2Expr(emptySetContainsToken(a), ...) } }} This of course is no longer tail-recursive
        * and therefore would require a stack-frame for every token that needs to be Expr[String]'ed. One possible alternative would be to trampoline the entire execution however,
        * that would likely introduce a significant performance penalty. Instead, a simplification can be made in which variations of the conditional (i.e. the regular expansion
        * and the emptySetContainsToken one) are expanded and they are kept separate in the 'done-pile' of token2Expr in some kind of data strucuture from which they can be picked
        * up later. The following sequence of steps therefore emerges when running into a `case Work.Token(SetContainsToken(a, op, b @ ScalarTagToken(tag)))` where `tag` is a list
        * lift: <li> Take the current done-area of token2Expr and stack a new matrowl above it <li> Process all the tokens that would be needed to apply a emptySetContainsToken
        * tokenization <li> Add yet another stack frame on top of the matrowl <li> Process all the tokens that would be needed to apply a regular tokenization of the list
        * i.e. `stmt"$a $op (") :: Work.AlreadyDone(liftsExpr) :: Work.Token(stmt")")` etc... and place them onto a the matrowl we just created. <li> Pop the two created stack
        * frames into groups (one, two) and splice them into the '{if (list.length != 0) {one} else {two}}` note that they will come out in the opposite order from which they were
        * put in.
        */
      ...
      extension (stringExprs: Seq[Expr[String]]) def mkStringExpr = stringExprs.foldLeft(Expr(""))((concatonation, nextExpr) => '{ $concatonation + $nextExpr })
      ...
      def token2Expr(token: Token): Expr[String] = {
        ...
        def apply(
            workList: List[Work],
            matrowl: Matrowl,
            placeholderCount: Expr[
              Int
            ] // I.e. the index of the '?' that is inserted in the query (that represents a lift) or the $N if an actual number is used (e.g. in the H2 context)
  ...
  object Dynamic:
    ...
    private[getquill] def raw[PrepareRowTemp, Session](
      ...
      val injectableLifts =
        lifts.collect { case e: InjectableEagerPlanter[_, _, _] => e.asInstanceOf[InjectableEagerPlanter[Any, PrepareRowTemp, Session]] }.map(lift => (lift.uid, lift)).toMap
