...
/**
 * For a query that has a filter(p =>
 * liftQuery(List("Joe","Jack")).contains(p.name)) we need to turn the "WHERE
 * p.name in (?)" into WHERE p.name in (?, ?) i.e. to "Particularize" the query
 * to the number of elements in the query lift. In Scala2-Quill we could just
 * access the values of the liftQuery list directly since the lift was an 'Any'
 * value directly in the AST. In Scala 3 however, we need to treat the lifted
 * list as an Expr and create an Expr[String] that represents the Query that is
 * to be during runtime based on the content of the list which has to be
 * manipulated inside of a '{ ... } block.
 */
object Particularize {
  ...
  object Static {
    ...
    /**
     * Convenience constructor for doing particularization from an
     * Unparticular.Query
     */
  ...
  private[getquill] class Dynamic[PrepareRowTemp, Session](traceConfig: TraceConfig) {
    ...
    def apply(
      ...
      val listLifts = lifts.collect { case e: EagerListPlanter[_, _, _] =>
        e.asInstanceOf[EagerListPlanter[Any, PrepareRowTemp, Session]]
      }.map(lift => (lift.uid, lift)).toMap
      val singleLifts = lifts.collect { case e: EagerPlanter[_, _, _] =>
        e.asInstanceOf[EagerPlanter[Any, PrepareRowTemp, Session]]
      }.map(lift => (lift.uid, lift)).toMap
      val injectableLifts = lifts.collect { case e: InjectableEagerPlanter[_, _, _] =>
        e.asInstanceOf[InjectableEagerPlanter[Any, PrepareRowTemp, Session]]
      }.map(lift => (lift.uid, lift)).toMap

      def getLifts(uid: String): LiftChoice =
        listLifts
          .get(uid)
          .map(LiftChoice.ListLift(_))
          ...
          if (workList.isEmpty) {
            ...
            head match {
              ...
              case Item(ScalarTagToken(tag)) =>
                ...
                apply(
                  tail,
                  sqlResult :+ liftPlaceholders,
                  lifts :+ newLift,
                  liftsCount + liftsLength,
                  valueClausesIndex
                )
              case Item(ValuesClauseToken(stmt)) =>
                val repeatedClauses =
                  (0 until valuesClauseRepeats).toChunk
                    .mapWithHasNext((i, hasNext) =>
                      List(SetValueClauseNum(i), Item(stmt), DoneValueClauseNum(i, !hasNext))
                    )
                    .flatten
              ...
              case Item(_: ScalarLiftToken) =>
                throw new UnsupportedOperationException(
                  "Scalar Lift Tokens are not used in Dotty Quill. Only Scalar Lift Tokens."
                )
  ...
  case class LiftsOrderer(slots: List[LiftSlot])(traceConfig: TraceConfig) {
    ...
    def orderLifts(valueClauseLifts: List[SingleEntityLifts], regularLifts: List[Planter[?, ?, ?]]) = {
      val valueClauseLiftIndexes =
        valueClauseLifts.zipWithIndex
          .flatMap((entity, i) => entity.lifts.map(lift => ValueLiftKey(i, lift.uid) -> lift))
          .toMap
      ...
      slots.map {
          ...
          valueClauseLiftIndexes
            ...
            .getOrElse {
              throw new IllegalStateException(
                s"Could not find the Value-Clause lift index:${valueClauseNum},uid:${uid}. Existing values are: ${valueClauseLiftIndexes}"
              )
            }
        case LiftSlot.Plain(uid) =>
          regularLiftIndexes
            ...
            .getOrElse {
              throw new IllegalStateException(
                s"Could not find the lift uid:${uid},uid:${uid}. Existing values are: ${regularLiftIndexes}"
              )
            }
        case other =>
  ...
  private[getquill] object UnparticularQueryLiftable {
    ...
    given liftUnparticularQuery: Lifters.Plain[Unparticular.Query] with {
      def lift = { case Unparticular.Query(basicQuery: String, realQuery: Statement) =>
  ...
  private[getquill] object StatementLiftable {
    ...
    given liftToken: Lifters.Plain[Token] with {
      def lift = {
        ...
        case ScalarTagToken(lift: ScalarTag) =>
          '{ io.getquill.idiom.ScalarTagToken(${ Lifter.NotSerializing.scalarTag(lift) }) }
        case QuotationTagToken(lift: QuotationTag) =>
          '{ io.getquill.idiom.QuotationTagToken(${ Lifter.NotSerializing.quotationTag(lift) }) }
        ...
        case SetContainsToken(a, op, b) =>
          '{ io.getquill.idiom.SetContainsToken(${ a.expr }, ${ op.expr }, ${ b.expr }) }
        case ScalarLiftToken(lift) =>
          quotes.reflect.report.throwError("Scalar Lift Tokens are not used in Dotty Quill. Only Scalar Lift Tokens.")
    ...
    given liftStatement: Lifters.Plain[Statement] with {
      def lift = { case Statement(tokens) =>
        '{ io.getquill.idiom.Statement(${ tokens.expr }) }
      }
    }
