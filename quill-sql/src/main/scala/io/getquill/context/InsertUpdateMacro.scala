...
/** TODO Right now this is just insert but we can easily extend to update and delete
  ...
  *
  * Once we've parsed an insert e.g. `query[Person].insert(Person("Joe", "Bloggs"))` we then need to synthesize the insertions that this would represent e.g.
  * `query[Person].insert(_.firstName -> "Joe", _.lastName -> "Bloggs")`
  *
  * Each function of field-insertion API basically takes the form {code} (v) => vAssignmentProperty -> assignmentValue (on the AST) {code}
  *
  * Let's take a look at a slighly more complex example Given: {code} case class Person(name: String, age: Option[Age]); Age(value: Int) quote { query[Person].insert(Person("Joe",
  * Age(345))) } {code}
  *
  * This expands out into a series of statements which will be parsed to AST assignments This: `(v: Person) => v.name -> (v:Person).name` Will be parsed into this: {code}
  * Assignment(Id(v), Prop(Id(v), name), Constant("Joe")) {code}
  *
  * This: `(v: Person) => v.age.map(v => v.value) -> Option(v:Age).map(v => v.value)` Will be parsed into this: {code} Assignment(Id(v), OptionTableMap(Prop(Id(v), age), Id(v),
  * Prop(Id(v), value)) OptionTableMap(OptionApply(CaseClass(value=345)), Id(v), Prop(Id(v), value)) ) {code}
  *
  * The end result of this synthesis is a series of assignments for an insert for the given entity.
  *
  * Another possiblity is that the entity is lifted: {code} case class Person(name: String, age: Option[Age]); Age(value: Int) quote { query[Person].insert(lift(Person("Joe",
  * Age(345)))) } {code} TODO Finish doc
  *
  * Note that as a result of the way this is implemented, if either the InsertMeta or the SchemaMeta is not inline, the entire resulting query will not be inline since they both
  * will be summoned and used in the resulting expressions. It might be useful to introduce a configuration parameter to ignore non-inline InsertMetas or non-inline SchemaMetas. Or
  * maybe this could even be an annotation.
  */
object InsertUpdateMacro {
  ...
  /** Perform the pipeline of creating an insert statement. The 'insertee' is the case class on which the SQL insert statement is based. The schema is based on the EntityQuery
    * which could potentially be an unquoted QuerySchema.
    */
  class Pipeline[T: Type, A[T] <: Insert[T] | Update[T]: Type](using override val qctx: Quotes) extends QuatMaking with QuatMakingBase(using qctx):
            ...
            QuotationLotExpr(actionMeta.asTerm.underlyingArgument.asExpr) match
              ...
              case Uprootable.Ast(ast) =>
                Unlifter(ast) match
                  ...
                  case other =>
                    report.throwError(
                      s"Invalid values in ${Format.TypeRepr(actionMeta.asTerm.tpe)}: ${other}. An ${Format.TypeRepr(actionMeta.asTerm.tpe)} AST must be a tuple of Property elements."
                    )
              ...
              case _ =>
                report.throwError(
                  s"The ${MacroType.asString}Meta form is invalid. It is Pointable: ${io.getquill.util.Format.Expr(actionMeta)}. It must be either Uprootable or Pluckable i.e. it has at least a UID that can be identified."
                )
    ...
    /** Inserted object can either be static: query[Person].insert(Person("Joe", "Bloggs")) or it can be lifted: query[Person].insert(lift(Person("Joe", "Bloggs")))
      *
      * In the later case, it will become: {{ //Assuming x := Person("Joe", "Bloggs") CaseClassLift( Quoted(ast: CaseClass(name -> lift(idA)), ...), lifts: List(EagerLift(x.name,
      * idA), ...)) ) }}
      *
      * For batch queries liftQuery(people).foreach(p => query[Person].insert(p)) it will be just the ast Ident("p")
      */
    ...
    /** Actually the same as deduceAssignmentsFromCaseClass, but I decided to write a separate function and comment it extensively since the logic is not simple to extrapolate.
      * This function creates a series of assignments of a elaborated product. However, each assignment just assigns to the identifier which will be plugged in (i.e. BetaReduced)
      * once the Ident is actually substituted. E.g. if we have something like this: `val ip = quote { (p: Person) => query[Person].insert(p) }` and then later:
      * `run(ip(lift(Person("Joe",123))))` then the assignments list is just based on the `p` identifier of the `ip` quoted function i.e: `(v:Person) => v.firstName -> p.firstName`
      * this is achived by doing BetaReduce(v.firstName, v -> p). Later on when `ip(lift(Person("Joe",123)))` happens the `CaseClass(firstName -> lift(...), age -> lift(...))`
    ...
    /** Create a static or dynamic quotation based on the state. Wrap the expr using some additional functions if we need to. This is used for the createFromPremade if we need to
      * wrap it into insertReturning which is used for batch-returning query execution.
      */
