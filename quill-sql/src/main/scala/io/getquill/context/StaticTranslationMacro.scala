...
object StaticTranslationMacro {
  ...
  private[getquill] def processAst[T: Type](astExpr: Expr[Ast], elaborate: ElaborationBehavior, idiom: Idiom, naming: NamingStrategy)(using
      Quotes
  ): Option[(Unparticular.Query, List[External], Option[ReturnAction], Ast)] =
  ...
  /** There are some cases where we actually do not want to use all of the lifts in a Quoted. For example: {{ query[Person].insert(_.id -> lift(1), _.name ->
    * lift("Joe")).returningGenerated(_.id)) }} becomes something like: {{ Quoted(query[Person].insert(_.id -> lift(A), _.name -> lift(B)).returningGenerated(_.id)), lifts:
    * List(ScalarTag(A, 1), ScalarTag(B, "Joe"))) }} but since we are excluding the person.id column (this is done in the transformation phase NormalizeReturning which is in
    * SqlNormalization in the quill-sql-portable module) actually we only want only the ScalarTag(B) so we need to get the list of lift tags (in tokens) once the Dialect has
    * serialized the query which correctly order the list of lifts. A similar issue happens with insertMeta and updateMeta. Process compile-time lifts, return `None` if that can't
    * be done. liftExprs = Lifts that were put into planters during the quotation. They are 're-planted' back into the PreparedStatement vars here. matchingExternals = the matching
    * placeholders (i.e 'lift tags') in the AST that contains the UUIDs of lifted elements. We check against list to make sure that that only needed lifts are used and in the right
    * order.
    */
