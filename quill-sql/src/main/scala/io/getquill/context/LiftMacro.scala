...
object LiftQueryMacro {
  ...
  def apply[T: Type, U[_] <: Iterable[_]: Type, PrepareRow: Type, Session: Type](
    entity: Expr[U[T]]
  )(using Quotes): Expr[Query[T]] = {
        ...
        val returning =
          '{
            EagerEntitiesPlanter(
              $entity.asInstanceOf[Iterable[T]],
              ${ Expr(newUuid) },
              ${ liftedLiftersExpr },
              ${ lifterClassExpr }
            ).unquote
...
object LiftMacro {
  ...
  inline def liftInjectedProductExternal[T, PrepareRow]: List[(String, T => Any)] = ${
    liftInjectedProductExternalImpl[T, PrepareRow]
  }
  def liftInjectedProductExternalImpl[T, PrepareRow](using
    qctx: Quotes,
    tpe: Type[T],
    prepareRowTpe: Type[PrepareRow]
  ): Expr[List[(String, T => Any)]] =
    Expr.ofList {
  ...
  private[getquill] def liftInjectedProduct[T, PrepareRow, Session](using
    qctx: Quotes,
    tpe: Type[T],
    prepareRowTpe: Type[PrepareRow],
    sessionTpe: Type[Session]
  ): (CaseClass, List[Expr[InjectableEagerPlanter[_, PrepareRow, Session]]]) = {
    import qctx.reflect._
    val (caseClassAstInitial, liftsInitial) = liftInjectedProductComponents[T, PrepareRow]
    val TaggedLiftedCaseClass(caseClassAst, lifts) =
      TaggedLiftedCaseClass(caseClassAstInitial, liftsInitial).reKeyWithUids()
    val liftPlanters =
      lifts.map((liftKey, lift) =>
        ...
        liftType match {
          case '[T => liftT] =>
            injectableLiftValue[liftT, PrepareRow, Session](
              lift.asExprOf[T => liftT],
              liftKey
            ) // Note: if want to get this to work, try doing 'summon[Type[liftT]]' (using liftType, prepareRowTpe, quotes)
        }
      )
    (caseClassAst, liftPlanters)
  }

  private[getquill] def liftInjectedProductComponents[T, PrepareRow](using
    qctx: Quotes,
    tpe: Type[T],
    prepareRowTpe: Type[PrepareRow]
  ): (CaseClass, List[(String, Expr[T => _])]) = {
    import qctx.reflect._
    import scala.quoted._
    import io.getquill.util.Format
    ...
    val liftLambdasAndTypes = ElaborateStructure.decomposedProductValue[T](
      ElaborationSide.Encoding
    ) // Elaboration side is 'Encoding' since we are in the lift macro
  ...
  private[getquill] def liftProduct[T, PrepareRow, Session](productEntity: Expr[T])(using
    qctx: Quotes,
    tpe: Type[T],
    prepareRowTpe: Type[PrepareRow],
    sessionTpe: Type[Session]
  ): Expr[CaseClassLift[T]] = {
    import qctx.reflect._
    ...
    val TaggedLiftedCaseClass(caseClassAst, lifts) =
      ElaborateStructure.ofProductValue[T](productEntity, ElaborationSide.Encoding).reKeyWithUids()
    val liftPlanters =
      lifts.map((liftKey, lift) =>
        // since we don't have an implicit Type for every single lift, we need to pull out each of their TypeReprs convert them to Type and manually pass them in
        // Also need to widen the type otherwise for some value v=Person(name: String) the type will be TermRef(TermRef(NoPrefix,val v),val name) as oppsoed to 'String'
        ...
        liftType match {
          case '[liftT] =>
            liftValue[liftT, PrepareRow, Session](
              lift.asExprOf[liftT],
              liftKey
            ) // Note: if want to get this to work, try doing 'summon[Type[liftT]]' (using liftType, prepareRowTpe, quotes)
        }
    ...
    '{
      CaseClassLift[T]($quotation, ${ Expr(java.util.UUID.randomUUID.toString) })
    } // NOTE UUID technically not needed here. Can try to remove it later
  }

  private[getquill] def summonEncoderOrFail[T: Type, PrepareRow: Type, Session: Type](loggingEntity: Expr[_])(using
    Quotes
  ) =
    import quotes.reflect._
    Expr.summon[GenericEncoder[T, PrepareRow, Session]] match
      ...
      case None =>
        report.throwError(
          s"Cannot Find a '${Printer.TypeReprCode.show(TypeRepr.of[T])}' Encoder of ${Printer.TreeShortCode
              .show(loggingEntity.asTerm)}",
          loggingEntity
        )

  private[getquill] def liftValue[T: Type, PrepareRow: Type, Session: Type](
    valueEntity: Expr[T],
    uuid: String = newUuid
  )(using Quotes) /*: Expr[EagerPlanter[T, PrepareRow]]*/ = {
    import quotes.reflect._
  ...
  def valueOrString[T: Type, PrepareRow: Type, Session: Type](valueEntity: Expr[Any], uuid: String = newUuid)(using
    Quotes
  ) =
    import quotes.reflect._
    ...
    val expectedClassTag =
      Expr.summon[ClassTag[T]] match
        ...
        case None =>
          report.throwError(
            s"Cannot create a classTag for the type ${Format.TypeOf[T]} for the value ${fieldName}. Cannot create a string-fallback encoder."
          )
  ...
  private[getquill] def injectableLiftValue[T: Type, PrepareRow: Type, Session: Type](
    valueEntity: Expr[_ => T],
    uuid: String = newUuid
  )(using Quotes) /*: Expr[EagerPlanter[T, PrepareRow]]*/ = {
    import quotes.reflect._
    val encoder =
      Expr.summon[GenericEncoder[T, PrepareRow, Session]] match
        ...
        case None =>
          report.throwError(
            s"Cannot inject the value: ${io.getquill.util.Format.Expr(valueEntity)}.Cannot Find a '${Printer.TypeReprCode
                .show(TypeRepr.of[T])}' Encoder of ${Printer.TreeShortCode.show(valueEntity.asTerm)}",
            valueEntity
          )

    '{
      InjectableEagerPlanter($valueEntity, $encoder, ${ Expr(uuid) })
    } // [T, PrepareRow] // adding these causes assertion failed: unresolved symbols: value Context_this
  }
  ...
  def applyLazy[T, PrepareRow](valueEntity: Expr[T])(using Quotes, Type[T], Type[PrepareRow]): Expr[T] = {
    ...
    '{
      LazyPlanter($valueEntity, ${ Expr(uuid) }).unquote
    } // [T, PrepareRow] // adding these causes assertion failed: unresolved symbols: value Context_this
  }
}
