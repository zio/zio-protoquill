...
/**
 * These are helper methods for doing various common tasks in cassandra. For
 * example, if you are filtering by non-indexed methods, use the
 * query.allowFiltering method to turn off the cassandra check. Note that this
 * will likely cause a full-table scan.
 *
 * Normally in Quill these are quoted however in ProtoQuill they are not always
 * unquoted e.g. when something like this happens: def peopleByName = quote {
 * (name: String) => people.filter(p => p.name == name).allowFiltering } since
 * the type Quoted[Q] for allowFiltering is not auto-unquoted, error will
 * happnen. This is why these are left as unquoted inline def.
 */
