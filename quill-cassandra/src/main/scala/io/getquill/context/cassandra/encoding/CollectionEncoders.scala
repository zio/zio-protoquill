...
trait CollectionEncoders extends EncodingDsl with CassandraRowContext {
  ...
  implicit def listEncoder[T, Cas](implicit
    ...
    encoder((index, list, row, session) =>
      row.setList[Cas](index.toInt, list.map(row => mapper.f(row, session)).asJava, asClassOf[Cas])
    )
  ...
  implicit def setEncoder[T, Cas](implicit
    ...
    encoder((index, list, row, session) =>
      row.setSet[Cas](index.toInt, list.map(row => mapper.f(row, session)).asJava, asClassOf[Cas])
    )
