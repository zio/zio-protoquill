...
object UdtDecodingMacro:
  ...
  private[UdtDecodingMacro] case class UdtParams[T <: Udt](
    udt: Expr[UdtValue],
    meta: Expr[UdtMeta[T]],
    sess: Expr[UdtValueLookup]
  )

  inline def udtDecoder[Decoder[_], T <: Udt]: CassandraDecoderMaker[Decoder, T] => Decoder[T] =
    ${ udtDecoderImpl[Decoder, T] }

  def udtDecoderImpl[Decoder[_]: Type, T <: Udt: Type](using
    Quotes
  ): Expr[CassandraDecoderMaker[Decoder, T] => Decoder[T]] = {
    ...
    if (TypeRepr.of[T] =:= TypeRepr.of[Udt])
      report.warning(
        s"Attempting to make UDT decoder for from ${Format.TypeOf[Decoder[T]]}:\n${ThreadUtil.currentThreadTrace}"
      )
  ...
  inline def udtDecodeMapper[Encoder[_], T <: Udt]
    : CassandraDecodeMapperMaker[Encoder, T] => CassandraMapper[UdtValue, T, MapperSide.Decode] = ${
    udtDecodeMapperImpl[Encoder, T]
  }

  def udtDecodeMapperImpl[Encoder[_]: Type, T <: Udt: Type](using
    Quotes
  ): Expr[CassandraDecodeMapperMaker[Encoder, T] => CassandraMapper[UdtValue, T, MapperSide.Decode]] =
    import quotes.reflect._
    val madeOrFoundMeta = UdtMeta.build[T]
  ...
  class UdtDecoderMaker[Encoder[_]: Type, T <: Udt: Type](using Quotes):
    ...
    def apply: UdtParams[T] => Expr[T] =
      (info: UdtParams[T]) => {
        ...
        def getField[C: Type](
          udtValue: Expr[UdtValue],
          fieldName: String,
          mapper: Expr[CassandraMapper[_, C, MapperSide.Decode]]
        ) =
        ...
        def getMap[CK: Type, CV: Type](udtValue: Expr[UdtValue], fieldName: String) =
          ...
          (keyMapper.asTerm.tpe.asType, valMapper.asTerm.tpe.asType) match
            case (
                  '[CassandraMapper[fromKT, CK, MapperSide.Decode]],
                  '[CassandraMapper[fromVT, CV, MapperSide.Decode]]
                ) =>
    ...
    def summonClassTagOrFail[CT: Type] =
      Expr.summon[ClassTag[CT]] match
        ...
        case None =>
          report.throwError(
            s"Error creating Encoder[${Format.TypeOf[T]}]. Cannot summon ClassTag for ${Format.TypeOf[CT]}"
          )
    ...
    def summonMapperOrFail[MT: Type]: Expr[CassandraMapper[_, MT, MapperSide.Decode]] =
      ...
      Expr.summon[CassandraMapper[_, MT, MapperSide.Decode]] match
        case Some(cm) => cm
        case None =>
          report.throwError(s"Error creating Encoder[${Format.TypeOf[T]}]. Cannot summon a CassandraMapper[${Format
              .TypeOf[MT]}, _, ${Format.TypeOf[MapperSide.Decode]}]")
  end UdtDecoderMaker
...
object UdtEncodingMacro:
  ...
  private[UdtEncodingMacro] case class UdtParams[T <: Udt: Type](
    elem: Expr[T],
    udt: Expr[UdtValue],
    meta: Expr[UdtMeta[T]],
    sess: Expr[UdtValueLookup]
  ...
  class UdtEncoderMaker[Encoder[_]: Type, T <: Udt: Type](using Quotes):
    ...
    def apply: (UdtParams[T] => Expr[UdtValue], TermType) = {
      val (deconstructedEntityComponents, elaborationType) =
        ElaborateStructure.decomposedProductValueDetails[T](ElaborationSide.Encoding, UdtBehavior.Derive)
      ...
      val udtMaker =
        (info: UdtParams[T]) => {
          def lookupField(name: String) =
            '{
              ${ info.meta }.alias(${ Expr(name) }).getOrElse(${ Expr(name) })
            } // todo insert summoned naming strategy here

          def setField[C: Type](
            fieldValue: Expr[C],
            fieldName: String,
            udt: Expr[UdtValue],
            mapper: Expr[CassandraMapper[C, _, MapperSide.Encode]]
          ) =
            mapper.asTerm.tpe.asType match
          ...
          // Elem is the elem type of the encoder. C is the component (i.e. udt-field) type
          def setOptional[Elem: Type, C: Type](
            fieldValue: Expr[Elem],
            fieldName: String,
            udt: Expr[UdtValue],
            getter: Expr[Elem] => Expr[?]
          ) =
            '{
              ${ getter.apply(fieldValue) }
                .asInstanceOf[Option[C]]
                .map(v => ${ setField[C]('v, fieldName, udt, summonMapperOrFail[C]) })
                .getOrElse($udt.setToNull(${ lookupField(fieldName) }))
            }

          def setRegular[Elem: Type, C: Type](
            fieldValue: Expr[Elem],
            fieldName: String,
            udt: Expr[UdtValue],
            getter: Expr[Elem] => Expr[?]
          ...
          def setList[Elem: Type, C: Type](
            ...
            mapper.asTerm.tpe.asType match
              case '[CassandraMapper[C, toT, MapperSide.Encode]] =>
                ...
                val list = '{
                  ${ getter.apply(fieldValue) }.asInstanceOf[List[C]].map(row => $typedMapper.f(row, ${ info.sess }))
                }
          ...
          def setSet[Elem: Type, C: Type](
            ...
            mapper.asTerm.tpe.asType match
              case '[CassandraMapper[C, toT, MapperSide.Encode]] =>
                val typedMapper = '{ $mapper.asInstanceOf[CassandraMapper[C, toT, MapperSide.Encode]] }
                val set = '{
                  ${ getter.apply(fieldValue) }.asInstanceOf[Set[C]].map(row => $typedMapper.f(row, ${ info.sess }))
                }
                val classToT = '{ ${ summonClassTagOrFail[toT] }.runtimeClass.asInstanceOf[Class[toT]] }
                '{ UdtValueOps($udt).setScalaSet[toT]($lookedUpField, $set, $classToT) }

          def setMap[Elem: Type, CK: Type, CV: Type](
            fieldValue: Expr[Elem],
            fieldName: String,
            udt: Expr[UdtValue],
            getter: Expr[Elem] => Expr[?]
            ...
            (keyMapper.asTerm.tpe.asType, valMapper.asTerm.tpe.asType) match
              case ('[CassandraMapper[CK, toKT, MapperSide.Encode]], '[CassandraMapper[CV, toVT, MapperSide.Encode]]) =>
                ...
                val map =
                  '{
                    ${ getter.apply(fieldValue) }
                      .asInstanceOf[Map[CK, CV]]
                      .map[toKT, toVT](kv =>
                        ($typedKeyMapper.f(kv._1, ${ info.sess }), $typedValMapper.f(kv._2, ${ info.sess }))
                      )
                  }
          ...
          val components =
            deconstructedEntityComponents.map { case (fieldName, isOptional, getter, fieldType) =>
              fieldType match
                case '[List[tpe]] =>
    ...
    def summonMapperOrFail[MT: Type]: Expr[CassandraMapper[MT, _, MapperSide.Encode]] =
      ...
      Expr.summon[CassandraMapper[MT, _, MapperSide.Encode]] match
        ...
        case None =>
          report.throwError(s"Error creating Encoder[${Format.TypeOf[T]}]. Cannot summon a CassandraMapper[${Format
              .TypeOf[MT]}, _, ${Format.TypeOf[MapperSide.Encode]}]")
  ...
  /**
   * Generate an Encoder[T] for some arbitrary Encoder[_] and arbitrary T <:
   * Udt. This is done via the Generic Derivation mechanisms of the ProtoQuill
   * Elaborators.
   *
   * Since we can't directly return an Encoder due to
   * https://github.com/lampepfl/dotty/issues/12179 (have a look at the AnyVal
   * encoder for more info), we pass it in as a parameter and the the
   * CassandraEncoderMaker pattern to generate it. If we assume that all the
   * encoders cassandra contexts ever need to use are CassandraEncoder instances
   * then we don't need this parameter and maybe can simplify. Need to look into
   * this some more.
   */
  def udtEncoderImpl[Encoder[_]: Type, T <: Udt: Type](using
    Quotes
  ): Expr[CassandraEncoderMaker[Encoder, T] => Encoder[T]] = {
    import quotes.reflect._
    import scala.deriving._
  ...
  inline def udtEncoderMapper[Encoder[_], T <: Udt]
    : CassandraEncodeMapperMaker[Encoder, T] => CassandraMapper[T, UdtValue, MapperSide.Encode] = ${
    udtEncoderMapperImpl[Encoder, T]
  }

  def udtEncoderMapperImpl[Encoder[_]: Type, T <: Udt: Type](using
    Quotes
  ): Expr[CassandraEncodeMapperMaker[Encoder, T] => CassandraMapper[T, UdtValue, MapperSide.Encode]] = {
    import quotes.reflect._
    val madeOrFoundMeta = UdtMeta.build[T]
    // TODO quill.trace.types 'summoning' level should enable this
