...
trait CassandraMapperConversions extends CassandraMapperConversionsLowPriorityImplicits {
  ...
  implicit def cassandraIdentityEncodeMapper[Cas](implicit
    cas: CassandraType[Cas]
  ): CassandraMapper[Cas, Cas, MapperSide.Encode] =
    CassandraMapper((i, _) => i)

  implicit def cassandraIdentityDecodeMapper[Cas](implicit
    cas: CassandraType[Cas]
  ): CassandraMapper[Cas, Cas, MapperSide.Decode] =
    CassandraMapper((i, _) => i)

  implicit def cassandraMapperEncode[T, Cas](implicit
    m: MappedEncoding[T, Cas],
    cas: CassandraType[Cas]
  ): CassandraMapper[T, Cas, MapperSide.Encode] = CassandraMapper((i, _) => m.f(i))

  implicit def cassandraMapperDecode[T, Cas](implicit
...
trait CassandraMapperConversionsLowPriorityImplicits {
  ...
  implicit def cassandraMapperEncodeRec[I, O, Cas](implicit
  ...
  implicit def cassandraMapperDecodeRec[I, O, Cas](implicit
