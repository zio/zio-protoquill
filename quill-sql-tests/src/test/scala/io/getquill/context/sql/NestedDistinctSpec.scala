  ...
  "nested distinct clauses should" - {
    ...
    "adversarial tests" - { //hellooo
      "should correctly rename the right property when multiple nesting layers have the same one" in {
        ...
        ctx.run(q).string mustEqual "SELECT DISTINCT 'Joe' AS name, norm.name, norm.id, mod.theName, mod.id FROM Emb norm INNER JOIN CustomEmb mod ON norm.name = mod.theName"
      }
    ...
    "query with multiple embedded elements with same names" - {
      ...
      "schema on the other one children should not override the other child or the parent" in {
        ...
        ctx.run(q).string mustEqual "SELECT DISTINCT 'Joe' AS name, mod.theName, mod.id, norm.name, norm.id FROM CustomEmb mod INNER JOIN Emb norm ON norm.name = mod.theName"
      }
      ...
      "schema on both of the children can be the same" in {
        ...
        ctx.run(q).string mustEqual "SELECT DISTINCT 'Joe' AS name, mod.theName, mod.id, norm.theName, norm.id FROM CustomEmb mod INNER JOIN CustomEmb norm ON norm.theName = mod.theName"
      }
      ...
      "schema on both of the children can be different" in {
        ...
        ctx.run(q).string mustEqual "SELECT DISTINCT 'Joe' AS name, mod.theSecondName, mod.id, norm.theFirstName, norm.id FROM CustomEmb mod INNER JOIN CustomEmb norm ON norm.theFirstName = mod.theSecondName"
      }
    }
  }
}
