...
object TypeclassUsecase_Encapsulated {
  ...
  def main(args: Array[String]): Unit = {
    ...
    inline def latestStatus[T, G](inline q: Query[T])(
      inline groupKey: T => G,
    ...
    inline def mastersLatest = quote {
      latestStatus(query[Master])(m => m.key, (a, b) => a.lastCheck < b.lastCheck)
    }
    inline def workersLatest = quote {
      latestStatus(query[Worker])(w => w.shard, (a, b) => a.lastTime < b.lastTime)
    }
