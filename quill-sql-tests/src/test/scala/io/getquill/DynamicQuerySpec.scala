...
class DynamicQuerySpec extends Spec { // //
  ...
  "query" - {
    ...
    inline def test[T](inline d: Quoted[Query[T]], inline s: Quoted[Query[T]]) =
    ...
    "filterOpt" - {
      "defined" in {
        val o = Some(1)
        test(
          // Need to do === or will get the following error:
          // Values of types Int and io.getquill.Quoted[Int] cannot be compared with == or !=
          dynamicQuery[TestEntity].filterOpt(o)((v0, i) => quote(v0.i === unquote(i))),
    ...
    "flat joins" - {
      ...
      // Not supported in for-comprehenion in ProtoQuill
      // "rightJoin" in {
      //   test(
      //     for {
      //       v0 <- dynamicQuery[TestEntity]
  ...
  "entityQuery" - {
    inline def test[T](inline d: Quoted[EntityQuery[T]], inline s: Quoted[EntityQuery[T]]) =
      testContext.run(d).string mustEqual testContext.run(s).string
    ...
    "filterOpt" - {
      "defined" in {
        val o = Some(1)
        test(
          dynamicQuery[TestEntity].filterOpt(o)((v0, i) => v0.i === unquote(i)),
          query[TestEntity].filter(v0 => v0.i == lift(1))
        )
      }
      "empty" in {
  ...
  "actions" - {
    inline def test[T](inline d: Quoted[Action[T]], inline s: Quoted[Action[T]]) =
      testContext.run(d).string mustEqual testContext.run(s).string

    // When ActionReturning is cast to action we get something like this:
    // java.lang.IllegalArgumentException: Invalid state. Cannot have None with a returning action Some(ReturnRecord)
    inline def testReturning[T, R](inline d: Quoted[ActionReturning[T, R]], inline s: Quoted[ActionReturning[T, R]]) =
      testContext.run(d).string mustEqual testContext.run(s).string
