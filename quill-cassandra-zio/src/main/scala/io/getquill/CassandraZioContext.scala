 ...
 * other contexts that require passing in a Data Source, this context takes in a
 * `ZioCassandraSession` as a resource dependency which can be provided later
 * (see the `ZioCassandraSession` object for helper methods that assist in doing
 * this).
 *
 * The resource dependency itself is just a Has[ZioCassandraSession]
 *
 * Various methods in the `io.getquill.ZioCassandraSession` can assist in
 * simplifying it's creation, for example, you can provide a `Config` object
 * instead of a `ZioCassandraSession` like this (note that the resulting
 * ZioCassandraSession has a closing bracket). {{ val zioSession =
 ...
 * }}
 *
 * ProtoQuill Note: Zio Cassandra context does not implement `prepare`. This it
 * can extend StandardContext in Scala2-Quill because presence of `prepare___`
 * methods is not enforced. Due to stricter type requirements in Dotty however,
...
class CassandraZioContext[+N <: NamingStrategy](val naming: N)
  ...
  ): ZIO[CassandraZioSession, Throwable, List[T]] = InternalApi.runQuery(quoted, wrap)
  @targetName("runQuerySingle")
  inline def run[T](inline quoted: Quoted[T]): ZIO[CassandraZioSession, Throwable, T] =
    InternalApi.runQuerySingle(quoted)
  ...
  ): ZIO[CassandraZioSession, Throwable, Unit] = InternalApi.runBatchAction(quoted, 1)
  ...
  private[getquill] def execute(cql: String, prepare: Prepare, csession: CassandraZioSession, fetchSize: Option[Int]) =
    simpleBlocking {
      prepareRowAndLog(cql, prepare).mapAttempt { p =>
        fetchSize match {
          case Some(value) => p.setPageSize(value)
          case None        => p
        }
      }.flatMap(p => ZIO.fromCompletionStage(csession.session.executeAsync(p)))
    }
  ...
  def executeAction(cql: String, prepare: Prepare = identityPrepare)(info: ExecutionInfo, dc: Runner): CIO[Unit] =
    simpleBlocking {
      for {
        csession <- ZIO.service[CassandraZioSession]
