...
object Quill {
  ...
  case class Cassandra[+N <: NamingStrategy](val naming: N, session: CassandraZioSession)
    ...
    inline def run[T](inline quoted: Quoted[Query[T]]): ZIO[Any, Throwable, List[T]] = InternalApi.runQueryDefault(quoted)
    @targetName("runQuery")
    inline def run[T](inline quoted: Quoted[Query[T]], inline wrap: OuterSelectWrap): ZIO[Any, Throwable, List[T]] = InternalApi.runQuery(quoted, wrap)
