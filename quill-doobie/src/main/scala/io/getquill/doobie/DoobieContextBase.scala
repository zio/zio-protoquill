  ...
  override def executeQuery[A](
    ...
    HC.prepareStatement(sql) {
      useConnection { implicit connection =>
        implicit val readInstance: Read[A] = extractorToRead(extractor)
  ...
  def streamQuery[A](
    ...
    for {
      ...
      result <-
        ...
        )(using extractorToRead(extractor)(connection))
    } yield result
    ...
    prepareConnections[List[A]](returningBehavior)(sql) {
      useConnection { implicit connection =>
        prepareAndLog(sql, prepare) *>
          ...
          {
            implicit val readInstance: Read[A] = extractorToRead(extractor)
            HPS.getGeneratedKeys[List[A]](HRS.list)
          }
      }
    }
