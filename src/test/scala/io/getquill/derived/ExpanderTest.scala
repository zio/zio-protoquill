...
class ExpanderTest extends Spec {
  import io.getquill.derived.Elaborate._
  val body = Ident("body")
  ...
  "correct expansion for" - {
    ...
    "Tuple" in {
      val ast = Elaborate.runtime[(String, String)](body)
      ast.toString  mustEqual  "body.map(x => (x._1, x._2))"
    }

    "Nested Tuple" in {
      case class Entity(a: String, b: Int)
      val ast = Elaborate.runtime[(String, Option[Entity])](body)
