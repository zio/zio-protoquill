...
object InlineMacroTest5_Typelevel {
  ...
  implicit def convString: ConvString = new ConvString
  implicit def convInt: ConvInt = new ConvInt

  inline def conv[T](inline element: T)(implicit inline conv: Conv[T]): conv.Out = {
    conv.get(element)
  }
